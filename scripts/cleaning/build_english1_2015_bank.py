#!/usr/bin/env python3
"""
Build the 2015 English I public-course bank from local Baidu Netdisk PDFs.

The original paper PDF is scanned, so this builder uses rendered page images
and OCR as question material evidence. The answer-speed PDF supplies the
official answer table, and the detailed-analysis PDF is retained as supporting
answer evidence.
"""
...
DEFAULT_OUTPUT = PROJECT_ROOT / "src" / "config" / "flashcard-banks" / "english1-2015.json"
DEFAULT_ASSET_DIR = PROJECT_ROOT / "cdn-assets" / "question-bank" / "english1-2015"
TMP_RENDER_DIR = PROJECT_ROOT / "tmp" / "pdfs" / "english1-2015-release-assets"

ORIGINAL_PAPER = RAW_DIR / "src_7a964264f77ea1398c793978-2015年考研英语一真题.pdf"
ANSWER_SPEED = RAW_DIR / "src_471b4dbc6486adfd5c97ed1e-2015年真题及答案速查.pdf"
DETAILED_ANALYSIS = RAW_DIR / "src_24e9cde64ed9e06971123d26-2015年真题逐题细解.pdf"

ORIGINAL_SOURCE_ID = "src_7a964264f77ea1398c793978"
ANSWER_SOURCE_ID = "src_471b4dbc6486adfd5c97ed1e"
DETAILED_SOURCE_ID = "src_24e9cde64ed9e06971123d26"
LOCAL_SOURCE_ID = "english1-2015-paper-answer"
...
TRANSLATION_ANSWERS = {
    46: "在各种强大动机的推动下，这场移民运动在荒野上创建了一个民族，且从本质上塑造了一个全新大陆的特点并决定了它的命运。",
    47: "合众国是两股主要力量作用的产物：一是带有不同思想、风俗习惯和民族特性的欧洲各民族的迁入，二是这个新国家因改变了这些特征而产生的影响。",
    48: "但是美洲特有地理条件的作用，不同族群之间的相互影响，以及在这片原始的新大陆上维持旧世界运行方式的巨大困难，所有这一切引发了意义重大的变化。",
    49: "十五、十六世纪，人们探索发现了北美洲，一百多年之后，第一批满载移民的航船横渡了大西洋，驶向今天合众国这片土地。",
    50: "树种丰富多样的原始森林从缅因州一直向南绵延至佐治亚州，可谓一个真正的宝库。",
}
...
WRITING_PROMPTS = {
    51: (
        ...
        "words recommending a book to the club members. State reasons for your "
        'recommendation. Do not sign your own name; use "Li Ming" instead.'
    ),
    52: (
        "Write an essay of 160-200 words based on the picture about a gathering "
        "in the mobile-phone era. Describe the picture briefly, interpret its "
        "intended meaning, and give your comments."
    ),
}
...
def image_ref(prefix: str, page: int, *, caption: str) -> dict[str, str]:
    ...
    return {
        "src": f"question-bank/english1-2015/{file_name}",
        "caption": caption,
        "alt": f"2015考研英语一{caption}",
    }


def ocr_page(asset_dir: Path, page: int) -> str:
    image = asset_dir / f"paper-page-{page:02d}.jpg"
    result = run(["tesseract", str(image), "stdout", "-l", "eng+chi_sim", "--psm", "6"], capture=True)
...
def build_passages(asset_dir: Path) -> dict[str, str]:
    ...
    if "statistical" not in passages["text3"].lower():
        raise RuntimeError("2015 Text 3 OCR did not contain the expected statistical-review article")
    if "reading proceed" not in passages["part-b"].lower():
        raise RuntimeError("2015 Part B OCR did not contain the expected reading-proceed passage")
    return passages
...
def explanation_for(number: int) -> str:
    if number <= 45:
        return "答案来自本地百度网盘 2015 年真题答案速查表，并用逐题细解 PDF 作为答案解析支持。"
    if number <= 50:
        return "参考译文来自本地百度网盘 2015 年真题答案速查表。"
...
def source_evidence(question_hash: str) -> dict[str, Any]:
    return {
        ...
        "verifiedBy": "build_english1_2015_bank",
    }
...
def answer_evidence(number: int, answer_hash: str) -> dict[str, Any]:
    ...
    return {
        ...
        "verifiedBy": "build_english1_2015_bank",
        "note": "Matched against local Baidu Netdisk 2015 English I answer-speed PDF; detailed analysis PDF is retained as supporting answer evidence.",
    }
...
def build_card(number: int, passages: dict[str, str]) -> dict[str, Any]:
    ...
    card = {
        "id": f"english1-2015-{number:03d}",
        "number": number,
        "paperId": "english1-2015",
        "paperName": "2015考研英语一真题",
        "subject": "英语",
        "subjectKey": "english",
        "track": "english1",
        "year": "2015",
        ...
        "tags": ["english1", "2015真题", section_for(number)],
...
def validate_cards(cards: list[dict[str, Any]]) -> None:
    ...
    if cards[30]["answer"] != "B" or "statistical" not in cards[30]["passage"].lower():
        issues.append("Text 3 evidence mismatch for question 31")
    if cards[40]["answer"] != "C" or "reading proceed" not in cards[40]["passage"].lower():
        issues.append("Part B evidence mismatch for question 41")
    if issues:
        raise ValueError("english1-2015 validation failed:\n" + "\n".join(issues))
...
def build_payload(*, asset_dir: Path, force_assets: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "sourceFiles": [
            {
                "id": "english1-2015-original-paper",
                "sourceId": ORIGINAL_SOURCE_ID,
                "role": "paper",
            ...
            {
                "id": "english1-2015-detailed-analysis",
        ...
        "assetRoot": "question-bank/english1-2015",
