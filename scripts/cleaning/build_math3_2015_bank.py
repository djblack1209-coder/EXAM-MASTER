#!/usr/bin/env python3
"""
Build the 2015 Math III public-course bank from the local Baidu Netdisk PDF.
...
DEFAULT_OUTPUT = PROJECT_ROOT / "src" / "config" / "flashcard-banks" / "math3-2015.json"
DEFAULT_ASSET_DIR = PROJECT_ROOT / "cdn-assets" / "question-bank" / "math3-2015"
TMP_RENDER_DIR = PROJECT_ROOT / "tmp" / "pdfs" / "math3-2015-release-assets"

PAPER_ANSWER = RAW_DIR / "src_08c2498ac7b58e3542942300-2015年考研数学三真题及解析.pdf"

SOURCE_IDS = {
    "paper_answer": "math3-2015-paper-answer",
}

PAPER_SOURCE_ID = "src_08c2498ac7b58e3542942300"
...
SHORT_ANSWERS = {
    15: "a=-1，b=-1/2，k=-1/3。",
    16: "二重积分值为 π/4 - 2/5。",
    17: "定价模型证明见答案原页；由 C(Q)=1600+Q^2、Q=40-P 得 P=30。",
    18: "f(x)=8/(4-x)。",
    19: "乘积求导公式证明及 n 个函数乘积求导公式见答案原页。",
    20: "a=0；X=[[3,1,-2],[1,1,-1],[2,1,-1]]。",
    21: "a=4，b=5；P=[[2,-3,-1],[1,0,-1],[0,1,1]]。",
    22: "P(Y=n)=(n-1)(1/8)^2(7/8)^(n-2), n=2,3,...；E(Y)=16。",
    23: "矩估计量 θ_hat=2X_bar-1；最大似然估计量 θ_hat=min{X1,X2,...,Xn}。",
}
...
def question_image_ref(crop_id: str) -> dict[str, str]:
    return {
        "src": f"question-bank/math3-2015/question-{crop_id}.jpg",
        "caption": f"题面裁切图 q{crop_id}",
        "alt": f"2015考研数学三第{crop_id.split('-')[0]}题题面裁切图",
    }


def answer_image_ref(page: int) -> dict[str, str]:
    return {
        "src": f"question-bank/math3-2015/answer-page-{page:02d}.jpg",
        "caption": f"答案解析 p.{page}",
        "alt": f"2015考研数学三答案解析 p.{page}",
    }
...
def question_anchor(number: int) -> str:
    return f"2015考研数学三第{number}题（{section_for(number)}）。题干、公式和图形以题面裁切图为准。"


def explanation_for(number: int) -> str:
    if number <= 8:
        return "答案来自本地百度网盘 2015 数学三真题及解析 PDF；选择题解析见答案原页图。"
    if number <= 14:
        return "答案来自本地百度网盘 2015 数学三真题及解析 PDF；完整题干、公式和解析见原页图。"
    return "答案来自本地百度网盘 2015 数学三真题及解析 PDF；完整演算、证明或推导过程见答案原页图。"


def source_evidence(number: int, answer_hash: str) -> dict[str, Any]:
    return {
        "evidenceId": f"math3-2015-q{number:02d}",
...
def build_card(number: int) -> dict[str, Any]:
    ...
    return {
        "id": f"math3-2015-{number:03d}",
        "number": number,
        "paperId": "math3-2015",
        "paperName": "2015考研数学三真题",
        "subject": "数学",
        "subjectKey": "math",
        "track": "math3",
        "year": "2015",
        ...
        "tags": ["math3", "2015", section_for(number)],
...
def validate_cards(cards: list[dict[str, Any]], *, asset_dir: Path) -> None:
    ...
    for expected_number, card in enumerate(cards, start=1):
        ...
        for image in [*card.get("questionImages", []), *card.get("answerImages", [])]:
            relative = str(image.get("src", "")).removeprefix("question-bank/math3-2015/")
    ...
    if issues:
        raise ValueError("math3-2015 validation failed:\n" + "\n".join(issues))
...
def build_payload(*, asset_dir: Path, force_assets: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "assetRoot": "question-bank/math3-2015",
