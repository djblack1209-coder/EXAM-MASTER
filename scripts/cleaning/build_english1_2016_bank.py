#!/usr/bin/env python3
"""
Build the 2016 English I public-course bank from local Baidu Netdisk PDFs.
...
DEFAULT_OUTPUT = PROJECT_ROOT / "src" / "config" / "flashcard-banks" / "english1-2016.json"
DEFAULT_ASSET_DIR = PROJECT_ROOT / "cdn-assets" / "question-bank" / "english1-2016"
TMP_RENDER_DIR = PROJECT_ROOT / "tmp" / "pdfs" / "english1-2016-release-assets"

ORIGINAL_PAPER = RAW_DIR / "src_1727b1f7ac7aed776cc06a4b-2016年考研英语一真题.pdf"
ANSWER_SPEED = RAW_DIR / "src_fd751ea21161f3c706d34b6d-2016年真题及答案速查.pdf"
DETAILED_ANALYSIS = RAW_DIR / "src_e9d1253a29a45721dba33b35-2016年真题逐题细解.pdf"

ORIGINAL_SOURCE_ID = "src_1727b1f7ac7aed776cc06a4b"
ANSWER_SOURCE_ID = "src_fd751ea21161f3c706d34b6d"
DETAILED_SOURCE_ID = "src_e9d1253a29a45721dba33b35"
LOCAL_SOURCE_ID = "english1-2016-paper-answer"
...
TRANSLATION_ANSWERS = {
    46: "我们无须学习如何保持心理健康；它是与生俱来的，正如我们的身体知道如何治愈伤口或修复骨折一样。",
    47: "心理健康其实一直都伴随着我们；宛如云朵背后的太阳，它虽然会暂时离开我们的视线，但又完全能瞬间重现。",
    48: "心理健康让我们在他人遇到困难时心怀同情，在他人遭受痛苦时心怀善念，并且无论对待任何人，都心怀无条件的关爱。",
    49: "尽管心理健康是我们生活的灵丹妙药，但它再寻常不过，因为你会发现，它一直指引你完成所有艰难的抉择。",
    50: "你会逐渐明白，懂得心理健康无时不在并懂得相信它，能让我们及时放慢节奏，快乐地生活。",
}
...
WRITING_PROMPTS = {
    51: (
        "Suppose you are a librarian in your university. Write a notice of about "
        "100 words, providing the newly-enrolled international students with "
        'relevant information about the library. Use "Li Ming" instead.'
    ),
    52: (
        "Write an essay of 160-200 words based on the pictures themed "
        "\"与其只提要求，不如做个榜样\". Describe the pictures briefly, "
        "interpret the meaning, and give your comments."
    ),
}
...
def image_ref(prefix: str, page: int, *, caption: str) -> dict[str, str]:
    ...
    return {
        "src": f"question-bank/english1-2016/{file_name}",
        "caption": caption,
        "alt": f"2016考研英语一{caption}",
    }
...
def build_passages(asset_dir: Path) -> dict[str, str]:
    ...
    passages = {
        ...
        "writing": section_between(text, r"\n\s*Section\s+H?I+\s+Writing", r"2016年考研英语"),
    }
    ...
    if "milton" not in text3 and "social responsibility" not in text3:
        raise RuntimeError("2016 Text 3 text layer did not contain the expected CSR article")
    if "image" not in part_b and "work environment" not in part_b:
        raise RuntimeError("2016 Part B text layer did not contain the expected image-upgrade passage")
    return passages
...
def explanation_for(number: int) -> str:
    if number <= 45:
        return "答案来自本地百度网盘 2016 年真题答案速查表，并用逐题细解 PDF 作为答案解析支持。"
    if number <= 50:
        return "参考译文来自本地百度网盘 2016 年真题答案速查表。"
...
def source_evidence(question_hash: str) -> dict[str, Any]:
    return {
        ...
        "verifiedBy": "build_english1_2016_bank",
    }
...
def answer_evidence(number: int, answer_hash: str) -> dict[str, Any]:
    ...
    return {
        ...
        "verifiedBy": "build_english1_2016_bank",
        "note": "Matched against local Baidu Netdisk 2016 English I answer-speed PDF; detailed analysis PDF is retained as supporting answer evidence.",
    }
...
def build_card(number: int, passages: dict[str, str]) -> dict[str, Any]:
    ...
    card = {
        "id": f"english1-2016-{number:03d}",
        "number": number,
        "paperId": "english1-2016",
        "paperName": "2016考研英语一真题",
        "subject": "英语",
        "subjectKey": "english",
        "track": "english1",
        "year": "2016",
        ...
        "tags": ["english1", "2016真题", section_for(number)],
...
def validate_cards(cards: list[dict[str, Any]]) -> None:
    ...
    if cards[30]["answer"] != "B" or "milton" not in cards[30]["passage"].lower():
        issues.append("Text 3 evidence mismatch for question 31")
    if cards[40]["answer"] != "C" or "work environment" not in cards[40]["passage"].lower():
        issues.append("Part B evidence mismatch for question 41")
    if issues:
        raise ValueError("english1-2016 validation failed:\n" + "\n".join(issues))
...
def build_payload(*, asset_dir: Path, force_assets: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "sourceFiles": [
            {
                "id": "english1-2016-original-paper",
                "sourceId": ORIGINAL_SOURCE_ID,
                "role": "paper",
            ...
            {
                "id": "english1-2016-detailed-analysis",
        ...
        "assetRoot": "question-bank/english1-2016",
