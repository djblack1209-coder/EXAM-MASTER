#!/usr/bin/env python3
"""Build the 2012 English II public-course flashcard bank."""
...
DEFAULT_OUTPUT = PROJECT_ROOT / "src" / "config" / "flashcard-banks" / "english2-2012.json"

DOCUMENT_VERSION = RAW_DIR / "src_c24aa84fb2c247d0b2d13bdc-2012.pdf"
ORIGINAL_PAPER = RAW_DIR / "src_8b9967de841a77398661e6c8-2012年考研英语二真题.pdf"
PAPER_ANSWER = RAW_DIR / "src_8b2fb9a4169220ee3ae4ca43-2012年真题及答案速查.pdf"
DETAILED_ANALYSIS = RAW_DIR / "src_edc0970329d767e5a81b2ff9-2012年真题逐题细解.pdf"

SOURCE_IDS = {
    "document_version": "src_c24aa84fb2c247d0b2d13bdc",
    "original_paper": "src_8b9967de841a77398661e6c8",
    "paper_answer": "src_8b2fb9a4169220ee3ae4ca43",
    "detailed_analysis": "src_edc0970329d767e5a81b2ff9",
}

CHOICE_ANSWERS = {
    **dict(zip(range(1, 6), "ABDAC")),
    **dict(zip(range(6, 11), "DBACB")),
    **dict(zip(range(11, 16), "DCCAD")),
    **dict(zip(range(16, 21), "ABCBD")),
    **dict(zip(range(21, 26), "ACDBA")),
    **dict(zip(range(26, 31), "CBBAC")),
    **dict(zip(range(31, 36), "ABACD")),
    **dict(zip(range(36, 41), "DDBDC")),
    **dict(zip(range(41, 46), "AFGCE")),
}
...
def require_sources() -> None:
    ...
    if missing:
        raise FileNotFoundError("Missing required English II 2012 source PDFs: " + ", ".join(missing))
...
def extract_cloze_passage(text: str) -> str:
    ...
    block = block.replace("who ] in World War II", "who (1) in World War II")
...
def extract_passages(text: str) -> dict[str, str]:
    passages = {
        "text1": cleaned_block(text_between(text, "Text 1", "21. It is implied in Paragraph 1")),
        "text2": cleaned_block(text_between(text, "Text 2", "26. By saying")),
        "text3": cleaned_block(text_between(text, "Text 3", "31. It can be learned from Paragraph 1")),
        "text4": cleaned_block(text_between(text, "Text 4", "36. By saying")),
        "part-b": cleaned_block(text_between(text, '"Universal history', "A. emphasized the virtue of classical heroes.")),
    }
    passages["part-b"] = f'"Universal history{passages["part-b"]}'
    return passages
...
def extract_translation_text(text: str) -> str:
    block = text_between(text, "When people in developing countries worry", "Section IV")
    return compact(f"When people in developing countries worry{block}")


def extract_translation_answer(text: str) -> str:
    block = text_between(text, "当发展中国家的人们担心移民问题时", "故事情节纯属虚构")
    return cleaned_block("当发展中国家的人们担心移民问题时" + block)


def extract_writing_prompts(text: str) -> dict[int, str]:
    prompt47 = cleaned_block(text_between(text, "47. Directions:", "英语（二）试题.13.（共14页）"))
    prompt48 = cleaned_block(text_between(text, "48. Directions:", "2012年考研英语（二）真题答案速查表"))
...
def attach_evidence(
    ...
    *,
    ...
    card["sourceEvidence"] = {
        ...
        "verifiedAt": now,
        "verifiedBy": "build_english2_2012_bank",
    }
...
def base_card(number: int, *, card_type: str, section: str, group_id: str, question: str, answer: str) -> dict[str, Any]:
    return {
        "id": f"english2-2012-{number:03d}",
        "paperId": "english2-2012",
        "paperName": "2012考研英语二真题",
        "subject": "英语",
        "subjectKey": "english",
        "track": "english2",
        "year": "2012",
...
def build_bank(now: str | None = None) -> dict[str, Any]:
    ...
    for number in range(1, 21):
        ...
        card["explanation"] = "答案已按本地百度网盘 2012 英语二真题答案速查表匹配，并用逐题细解 PDF 复核。"
        cards.append(
            attach_evidence(
                card,
                ...
                answer_note="Matched against the local 2012 English II answer-speed PDF.",
            )
        )
    ...
    for number in range(21, 41):
        ...
        card["explanation"] = "答案已按本地百度网盘 2012 英语二答案速查表匹配，并用逐题细解 PDF 阅读解析复核。"
        cards.append(
            attach_evidence(
                card,
                question_source_id=SOURCE_IDS["paper_answer"],
    ...
    for number in range(41, 46):
        ...
        card["explanation"] = "答案已按本地百度网盘 2012 英语二答案速查表匹配；Part B 按 A-G 信息匹配题训练。"
        cards.append(
            attach_evidence(
                card,
                question_source_id=SOURCE_IDS["paper_answer"],
    ...
    translation["explanation"] = "参考译文已按本地百度网盘 2012 英语二答案速查 PDF 匹配，并与逐题细解 PDF 英汉对照复核。"
    cards.append(
        attach_evidence(
            translation,
            question_source_id=SOURCE_IDS["paper_answer"],
    ...
    return {
        ...
        "sourceFiles": [
            {
                "id": "english2-2012-document-version",
            ...
            {
                "id": "english2-2012-original-paper",
            ...
            {
                "id": "english2-2012-detailed-analysis",
...
def validate_cards(cards: list[dict[str, Any]], material: dict[str, Any], text: str, detail_text: str) -> None:
    ...
    if "Homework" not in material["passages"]["text1"] or "L. A. Unified" not in material["passages"]["text1"]:
        issues.append("Text 1 passage missing Homework/L. A. Unified anchors")
    if "brain drain" not in material["translation_text"] or "developing countries" not in material["translation_text"]:
        issues.append("translation text missing brain drain/developing countries anchors")
    if "某公司员工工作满意度调查" not in material["writing_prompts"][48]:
        issues.append("writing prompt 48 missing employee work satisfaction survey anchor")
    if "ABDAC" not in text or "AFGCE" not in text:
        issues.append("answer-speed table anchors missing")
    if "答案解析" not in detail_text or "G. I. Joe" not in detail_text or "brain drain" not in detail_text:
    ...
    if not cards[45]["answer"].startswith("当发展中国家的人们担心移民问题时"):
        issues.append("question 46 translation answer mismatch")
    if len(cards[40]["options"]) != 7 or cards[40]["answer"] != "A":
    ...
    if issues:
        raise ValueError("english2-2012 validation failed:\n" + "\n".join(issues))
