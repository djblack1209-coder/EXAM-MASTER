#!/usr/bin/env python3
"""
Build the 2018 Math II public-course bank from the local Baidu Netdisk PDF.
...
DEFAULT_OUTPUT = PROJECT_ROOT / "src" / "config" / "flashcard-banks" / "math2-2018.json"
DEFAULT_ASSET_DIR = PROJECT_ROOT / "cdn-assets" / "question-bank" / "math2-2018"
TMP_RENDER_DIR = PROJECT_ROOT / "tmp" / "pdfs" / "math2-2018-release-assets"

PAPER_ANSWER = RAW_DIR / "src_a91bd4ebb8ae4c75b320a8a3-2018考研数学二真题.pdf"

SOURCE_IDS = {
    "paper_answer": "math2-2018-paper-answer",
}

PAPER_SOURCE_ID = "src_a91bd4ebb8ae4c75b320a8a3"
...
SHORT_ANSWERS = {
    15: "不定积分为 1/2(e^{2x} arctan sqrt(e^x-1)-1/3(e^x-1)^{3/2}-sqrt(e^x-1))+C。",
    16: "(I) f(x)=e^{-x}(2ae^x-2a)；(II) a=e/2。",
    17: "二重积分值为 3π²+5π。",
    18: "不等式证明见答案原页。",
    19: "存在最小值，最小面积为 1/(π+4+3√3)。",
    20: "10",
    21: "数列收敛，lim x_n=0。",
    22: "a=2 时 f=0 有非零解，通解 x=k(2,1,-1)^T；规范形见答案原页。",
    23: "a=2；满足 AP=B 的可逆矩阵 P 见答案原页。",
}
...
def question_image_ref(crop_id: str) -> dict[str, str]:
    return {
        "src": f"question-bank/math2-2018/question-{crop_id}.jpg",
        "caption": f"题面裁切图 q{crop_id}",
        "alt": f"2018考研数学二第{crop_id.split('-')[0]}题题面裁切图",
    }


def answer_image_ref(page: int) -> dict[str, str]:
    return {
        "src": f"question-bank/math2-2018/answer-page-{page:02d}.jpg",
        "caption": f"答案解析 p.{page}",
        "alt": f"2018考研数学二答案解析 p.{page}",
    }
...
def question_anchor(number: int) -> str:
    return f"2018考研数学二第{number}题（{section_for(number)}）。题干、公式和图形以题面裁切图为准。"


def explanation_for(number: int) -> str:
    if number <= 8:
        return "答案来自本地百度网盘 2018 数学二真题及解析 PDF；选择题解析见答案原页图。"
    if number <= 14:
        return "答案来自本地百度网盘 2018 数学二真题及解析 PDF；完整题干、公式和解析见原页图。"
    return "答案来自本地百度网盘 2018 数学二真题及解析 PDF；完整演算、证明或推导过程见答案原页图。"


def source_evidence(number: int, answer_hash: str) -> dict[str, Any]:
    return {
        "evidenceId": f"math2-2018-q{number:02d}",
...
def build_card(number: int) -> dict[str, Any]:
    ...
    return {
        "id": f"math2-2018-{number:03d}",
        "number": number,
        "paperId": "math2-2018",
        "paperName": "2018考研数学二真题",
        "subject": "数学",
        "subjectKey": "math",
        "track": "math2",
        "year": "2018",
        ...
        "tags": ["math2", "2018", section_for(number)],
...
def validate_cards(cards: list[dict[str, Any]], *, asset_dir: Path) -> None:
    ...
    for expected_number, card in enumerate(cards, start=1):
        ...
        for image in [*card.get("questionImages", []), *card.get("answerImages", [])]:
            relative = str(image.get("src", "")).removeprefix("question-bank/math2-2018/")
    ...
    if issues:
        raise ValueError("math2-2018 validation failed:\n" + "\n".join(issues))
...
def build_payload(*, asset_dir: Path, force_assets: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "assetRoot": "question-bank/math2-2018",
