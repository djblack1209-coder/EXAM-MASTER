#!/usr/bin/env python3
"""
Build the 2019 Math III public-course bank from the local Baidu Netdisk PDF.

The source is a scanned answer-analysis PDF without a usable text layer. Its
...
DEFAULT_OUTPUT = PROJECT_ROOT / "src" / "config" / "flashcard-banks" / "math3-2019.json"
DEFAULT_ASSET_DIR = PROJECT_ROOT / "cdn-assets" / "question-bank" / "math3-2019"
TMP_RENDER_DIR = PROJECT_ROOT / "tmp" / "pdfs" / "math3-2019-release-assets"

PAPER_ANSWER = RAW_DIR / "src_72f4c3221cc958781119c8d1-2019年考研数学三真题及解析.pdf"

SOURCE_IDS = {
    "paper_answer": "math3-2019-paper-answer",
}

PAPER_SOURCE_ID = "src_72f4c3221cc958781119c8d1"
...
SHORT_ANSWERS = {
    15: "f'(x) 分段表达式见答案原页；f(x) 在 x=0 取极大值 1。",
    16: "所求二阶偏导组合结果为 1 - 3f_11'' - f_22''。",
    17: "y=sqrt(x)e^(x^2/2)；旋转体体积 V=π/2(e^4-4)。",
    18: "所围图形面积为 1/2 + 1/(e^π - 1)。",
    19: "{a_n} 单调递减，a_n=(n-1)/(n+2)a_{n-2}；a_n/a_{n-1} 的奇偶子列极限见答案原页。",
    20: "a=1；β3=(-k-1)α1+kα2+2α3，k∈R。",
    21: "x=3，y=-2；P=[[-1,-1,-1],[2,1,2],[0,0,4]]。",
    22: "Z 的概率密度见答案原页；p=1/2 时 X 与 Z 不相关；X 与 Z 不独立。",
    23: "A=sqrt(2/π)；σ^2 的最大似然估计量为 n^-1 sum((X_i-μ)^2)。",
}
...
def question_image_ref(crop_id: str) -> dict[str, str]:
    return {
        "src": f"question-bank/math3-2019/question-{crop_id}.jpg",
        "caption": f"题面裁切图 q{crop_id}",
        "alt": f"2019考研数学三第{crop_id.split('-')[0]}题题面裁切图",
    }


def answer_image_ref(page: int) -> dict[str, str]:
    return {
        "src": f"question-bank/math3-2019/answer-page-{page:02d}.jpg",
        "caption": f"答案解析 p.{page}",
        "alt": f"2019考研数学三答案解析 p.{page}",
    }
...
def question_anchor(number: int) -> str:
    return f"2019考研数学三第{number}题（{section_for(number)}）。题干、公式和图形以题面裁切图为准。"


def explanation_for(number: int) -> str:
    if number <= 8:
        return "答案来自本地百度网盘 2019 数学三真题及解析 PDF；选择题解析见答案原页图。"
    if number <= 14:
        return "答案来自本地百度网盘 2019 数学三真题及解析 PDF；完整题干、公式和解析见原页图。"
    return "答案来自本地百度网盘 2019 数学三真题及解析 PDF；完整演算、证明或推导过程见答案原页图。"


def source_evidence(number: int, answer_hash: str) -> dict[str, Any]:
    return {
        "evidenceId": f"math3-2019-q{number:02d}",
...
def build_card(number: int) -> dict[str, Any]:
    ...
    return {
        "id": f"math3-2019-{number:03d}",
        "number": number,
        "paperId": "math3-2019",
        "paperName": "2019考研数学三真题",
        "subject": "数学",
        "subjectKey": "math",
        "track": "math3",
        "year": "2019",
        ...
        "tags": ["math3", "2019", section_for(number)],
...
def validate_cards(cards: list[dict[str, Any]], *, asset_dir: Path) -> None:
    ...
    if issues:
        raise ValueError("math3-2019 validation failed:\n" + "\n".join(issues))


def build_payload(*, asset_dir: Path, force_assets: bool) -> dict[str, Any]:
    render_pdf_assets(PAPER_ANSWER, "answer-page", asset_dir=asset_dir, force=force_assets, pages=set(range(1, 13)))
        ...
        "sourceFiles": [
            {
                ...
                "sourceId": PAPER_SOURCE_ID,
                "evidenceNote": "The 12-page source interleaves each question with its answer analysis; page 12 is the continuation of q23 answer evidence, not an ad-only page.",
            },
        ],
        "assetRoot": "question-bank/math3-2019",
