#!/usr/bin/env python3
"""
Build the 2005 Math II public-course bank from the local Baidu Netdisk PDF.
...
RAW_DIR = PROJECT_ROOT / "data" / "raw-inbox" / "public-course-history" / "math2" / "2005"
DEFAULT_OUTPUT = PROJECT_ROOT / "src" / "config" / "flashcard-banks" / "math2-2005.json"
DEFAULT_ASSET_DIR = PROJECT_ROOT / "cdn-assets" / "question-bank" / "math2-2005"
TMP_RENDER_DIR = PROJECT_ROOT / "tmp" / "pdfs" / "math2-2005-release-assets"

PAPER_ANSWER = RAW_DIR / "2005-math2-paper-answer.pdf"

SOURCE_IDS = {
    "paper_answer": "math2-2005-paper-answer",
}

PAPER_SOURCE_ID = "src_f888957d448607a835bec76d"

FILL_ANSWERS = {
    1: "-pi dx",
    2: "y = x + 3/2",
    3: "pi/4",
    4: "y = 1/3 x ln x - 1/9 x",
    5: "3/4",
    6: "2",
}
...
SHORT_ANSWERS = {
    ...
    17: "20",
    18: "y = 2x + sqrt(1 - x^2)",
    19: "存在 ξ 使得 f(ξ) = 1 - ξ；且存在两个不同的点 η, ξ 使得 f'(η)f'(ξ) = 1。",
    20: "z = x^2 - y^2 + 2；驻点 (0,0) 的函数值为 2；边界最大值 3，最小值 -2。",
    21: "积分值为 pi/4 - 1/3。",
    22: "a = 1。",
    23: "若 r(A)=2，则 k=9，Ax=0 的通解为 t(1,2,3)^T；若 r(A)=1，则由 a+2b+3c=0 且 (k-9)c=0 确定，c!=0 时通解为 t1(c,0,-a)^T+t2(0,c,-b)^T，c=0 时通解为 t1(1,2,0)^T+t2(0,0,1)^T。",
}
...
def image_ref(kind: str, page: int, *, caption: str) -> dict[str, str]:
    ...
    return {
        "src": f"question-bank/math2-2005/{file_name}",
        "caption": caption,
        "alt": f"2005考研数学二{caption}",
    }
...
def question_anchor(number: int) -> str:
    return f"2005考研数学二第{number}题（{section_for(number)}）。题干、公式和图形以试卷原页图为准。"


def explanation_for(number: int) -> str:
    if number <= 6:
        return "答案来自本地百度网盘 2005 数学二真题及解析 PDF；完整题干和公式见试卷原页图。"
    if number <= 14:
        return "答案来自本地百度网盘 2005 数学二真题及解析 PDF；选择题解析见答案原页图。"
    return "答案来自本地百度网盘 2005 数学二真题及解析 PDF；完整演算、证明或推导过程见答案原页图。"


def source_evidence(number: int, answer_hash: str) -> dict[str, Any]:
    return {
        "evidenceId": f"math2-2005-q{number:02d}",
...
def build_card(number: int) -> dict[str, Any]:
    ...
    return {
        "id": f"math2-2005-{number:03d}",
        "number": number,
        "paperId": "math2-2005",
        "paperName": "2005考研数学二真题",
        "subject": "数学",
        "subjectKey": "math",
        "track": "math2",
        "year": "2005",
        ...
        "tags": ["math2", "2005", section_for(number)],
...
def build_payload(*, asset_dir: Path, force_assets: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "assetRoot": "question-bank/math2-2005",
