#!/usr/bin/env python3
"""
Build the 2018 Math III public-course bank from the local Baidu Netdisk PDF.
...
DEFAULT_OUTPUT = PROJECT_ROOT / "src" / "config" / "flashcard-banks" / "math3-2018.json"
DEFAULT_ASSET_DIR = PROJECT_ROOT / "cdn-assets" / "question-bank" / "math3-2018"
TMP_RENDER_DIR = PROJECT_ROOT / "tmp" / "pdfs" / "math3-2018-release-assets"

PAPER_ANSWER = RAW_DIR / "src_96a8e7cbc8f19d8e26451784-2018年考研数学三真题及解析.pdf"

SOURCE_IDS = {
    "paper_answer": "math3-2018-paper-answer",
}

PAPER_SOURCE_ID = "src_96a8e7cbc8f19d8e26451784"
...
FILL_ANSWERS = {
    9: "y=4x-3",
    10: "e^x arccos(e^x) - sqrt(1-e^(2x)) + C",
    11: "y_x = C1 + C2 2^x - 5",
    12: "2e",
    13: "2",
    14: "1/3",
...
SHORT_ANSWERS = {
    15: "a=1，b=1。",
    16: "二重积分值为 sqrt(3)π/32 - sqrt(3)/16。",
    17: "最小面积为 1/(π+4+3sqrt(3))。",
    18: "a_n: n 为奇数时 n+1；n 为偶数时 (-1)^(n/2)2^n/n! - n - 1。",
    19: "数列收敛，lim x_n = 0。",
    20: "a=2 时 f=0 有非零解，通解 x=k(2,1,-1)^T；规范形见答案原页。",
    21: "a=2；满足 AP=B 的可逆矩阵 P 见答案原页。",
    22: "Cov(X,Z)=λ；Z 的分布律见答案原页。",
    23: "σ 的最大似然估计量为 sum(|X_i|)/n，Eσhat 与 Dσhat 见答案原页。",
}
...
def question_image_ref(crop_id: str) -> dict[str, str]:
    return {
        "src": f"question-bank/math3-2018/question-{crop_id}.jpg",
        "caption": f"题面裁切图 q{crop_id}",
        "alt": f"2018考研数学三第{crop_id.split('-')[0]}题题面裁切图",
    }


def answer_image_ref(page: int) -> dict[str, str]:
    return {
        "src": f"question-bank/math3-2018/answer-page-{page:02d}.jpg",
        "caption": f"答案解析 p.{page}",
        "alt": f"2018考研数学三答案解析 p.{page}",
    }
...
def question_anchor(number: int) -> str:
    return f"2018考研数学三第{number}题（{section_for(number)}）。题干、公式和图形以题面裁切图为准。"


def explanation_for(number: int) -> str:
    if number <= 8:
        return "答案来自本地百度网盘 2018 数学三真题及解析 PDF；选择题解析见答案原页图。"
    if number <= 14:
        return "答案来自本地百度网盘 2018 数学三真题及解析 PDF；完整题干、公式和解析见原页图。"
    return "答案来自本地百度网盘 2018 数学三真题及解析 PDF；完整演算、证明或推导过程见答案原页图。"


def source_evidence(number: int, answer_hash: str) -> dict[str, Any]:
    return {
        "evidenceId": f"math3-2018-q{number:02d}",
...
def build_card(number: int) -> dict[str, Any]:
    ...
    return {
        "id": f"math3-2018-{number:03d}",
        "number": number,
        "paperId": "math3-2018",
        "paperName": "2018考研数学三真题",
        "subject": "数学",
        "subjectKey": "math",
        "track": "math3",
        "year": "2018",
        ...
        "tags": ["math3", "2018", section_for(number)],
...
def validate_cards(cards: list[dict[str, Any]], *, asset_dir: Path) -> None:
    ...
    if issues:
        raise ValueError("math3-2018 validation failed:\n" + "\n".join(issues))
        ...
        "sourceFiles": [
            {
                ...
                "sourceId": PAPER_SOURCE_ID,
                "evidenceNote": "The 14-page source interleaves each question with its answer analysis; page 14 is the continuation of q23 answer evidence, not an ad-only page.",
            },
        ],
        "assetRoot": "question-bank/math3-2018",
