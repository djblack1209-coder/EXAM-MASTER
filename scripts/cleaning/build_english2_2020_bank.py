#!/usr/bin/env python3
"""Build the 2020 English II public-course flashcard bank."""
...
DEFAULT_OUTPUT = PROJECT_ROOT / "src" / "config" / "flashcard-banks" / "english2-2020.json"

DOCUMENT_VERSION = RAW_DIR / "src_efd6104a55adbacf4339a6d1-2020.pdf"
ORIGINAL_PAPER = RAW_DIR / "src_8d78ccec060de76134b13cbf-2020年考研英语二真题.pdf"
PAPER_ANSWER = RAW_DIR / "src_e2a5233040d61414cb4a4589-2020年真题及答案速查.pdf"
DETAILED_ANALYSIS = RAW_DIR / "src_ca2554877698dac7a218b6e9-2020年真题逐题细解.pdf"

SOURCE_IDS = {
    "document_version": "src_efd6104a55adbacf4339a6d1",
    "original_paper": "src_8d78ccec060de76134b13cbf",
    "paper_answer": "src_e2a5233040d61414cb4a4589",
    "detailed_analysis": "src_ca2554877698dac7a218b6e9",
}

CHOICE_ANSWERS = {
    **dict(zip(range(1, 6), "BCADD")),
    **dict(zip(range(6, 11), "CBDAB")),
    **dict(zip(range(11, 16), "ADBCD")),
    **dict(zip(range(16, 21), "DACBC")),
    **dict(zip(range(21, 26), "ADBCD")),
    **dict(zip(range(26, 31), "CDBDA")),
    **dict(zip(range(31, 36), "DDADB")),
    **dict(zip(range(36, 41), "CBCAC")),
    **dict(zip(range(41, 46), "EFDAC")),
}
...
WRITING_CHART_48 = (
    "Chart: 某高校学生手机阅读目的调查（学习知识59.5%；消磨时间21.3%；获取信息17.0%；其他2.2%）."
)
...
def compact(value: Any) -> str:
    ...
    replacements = {
        ...
        "U. S.": "U.S.",
        "Jos6": "Jose",
        "Ifs vital": "It's vital",
        "Ifs not": "It's not",
        "Ifs common": "It's common",
        "ifs entirely": "it's entirely",
        "If s almost": "It's almost",
        "If s": "It's",
        "others, shoes": "others' shoes",
        "u school streets\"": '"school streets"',
        "u Millennials": '"Millennials',
        "averse. \"": 'averse."',
        ...
        "lar ge": "large",
        "suppl y": "supply",
        "underl ying": "underlying",
        "graȧuating": "graduating",
        ...
        "fıom": "from",
        "the y": "they",
        "m a genuine": "in a genuine",
        "certain ty": "certainty",
        "troubles seeing": "trouble seeing",
        "motive inside": "motivate inside",
        "work-life \u4ea7 balance": "work-life balance",
        "•.；二.菸“ e 』": "",
        "C.reach": "C. reach",
        "C.indeed": "C. indeed",
        "child9s": "child's",
...
def require_sources() -> None:
    ...
    if missing:
        raise FileNotFoundError("Missing required English II 2020 source PDFs: " + ", ".join(missing))
...
def page_footer_pattern() -> re.Pattern[str]:
    return re.compile(r"英语[（(]二[）)]试题[,.，]\s*\d+\s*[,.，][（(]共14页[）)]")
...
def extract_cloze_passage(text: str) -> str:
    ...
    block = re.sub(r"undoubtedly very\s+1\s+,", "undoubtedly very (1),", block, count=1)
...
def extract_cloze_options(text: str) -> dict[int, dict[str, str]]:
    option_text = text_between(text, "英语（二）试题.1.（共14页）", "Section II       Reading Comprehension")
    options: dict[int, dict[str, str]] = {}
    for number in range(1, 21):
        match = re.search(rf"(?m)^\s*(?:[A-Za-z]{{1,3}}\s+)?{number}\.\s+(.+)$", option_text)
        if not match:
...
def extract_passages(text: str) -> dict[str, str]:
    passages = {
        "text1": cleaned_block(text_between(text, "Text 1", "21. Quinn and her colleagues")),
        "text2": cleaned_block(text_between(text, "Text 2", "26. Which of the following")),
        "text3": cleaned_block(text_between(text, "Text 3", "31. Which of the following")),
        "text4": cleaned_block(text_between(text, "Text 4", "36. Generation Zs")),
        "part-b": cleaned_block(text_between(text, "Five Ways to Win Over Everyone in the Office", "Section ID")),
    }
    passages["part-b"] = f"Five Ways to Win Over Everyone in the Office {passages['part-b']}"
    return passages
...
def parse_choice_question(block: str, number: int) -> tuple[str, dict[str, str]]:
    ...
    normalized = re.sub(r"\s+「・\s+上\s+i\s+", " ", normalized)
    normalized = re.sub(r"\s+，'去泠\.'病:J\s*", " ", normalized)
    normalized = re.sub(r"\s+产\s+balance\s+•\.；二\.菸“\s+e\s+』", " balance", normalized)
...
def extract_translation_text(text: str) -> str:
    block = text_between(text, "If s almost impossible to go through life", "Section IV")
    return compact(page_footer_pattern().sub("", f"If s almost impossible to go through life{block}"))


def extract_translation_answer(text: str) -> str:
    block = text_between(text, "人的一生几乎不可能不经历某种失败", "英语（二）试题.14.（共14页）")
    return cleaned_block("人的一生几乎不可能不经历某种失败" + block)


def extract_writing_prompts(text: str) -> dict[int, str]:
    writing_section = text_between(text, "Section IV      Writing", "2020年考研英语（二）真题答案速查表")
    prompt47 = cleaned_block(text_between(writing_section, "47. Directions:", "PartB"))
    prompt48 = cleaned_block(text_between(writing_section, "48. Directions:", "某高校学生手机阅读目的调查"))
...
def attach_evidence(
    ...
    *,
    ...
    card["sourceEvidence"] = {
        ...
        "verifiedAt": now,
        "verifiedBy": "build_english2_2020_bank",
    }
...
def base_card(number: int, *, card_type: str, section: str, group_id: str, question: str, answer: str) -> dict[str, Any]:
    return {
        "id": f"english2-2020-{number:03d}",
        "paperId": "english2-2020",
        "paperName": "2020考研英语二真题",
        "subject": "英语",
        "subjectKey": "english",
        "track": "english2",
        "year": "2020",
...
def build_bank(now: str | None = None) -> dict[str, Any]:
    ...
    for number in range(1, 21):
        ...
        card["explanation"] = "答案已按本地百度网盘 2020 英语二真题答案速查表匹配，并用逐题细解 PDF 复核。"
        cards.append(
            attach_evidence(
                card,
                ...
                answer_note="Matched against the local 2020 English II answer-speed PDF.",
            )
        )
    ...
    for number in range(21, 41):
        ...
        card["explanation"] = "答案已按本地百度网盘 2020 英语二答案速查表匹配，并用逐题细解 PDF 阅读解析复核。"
        cards.append(
            attach_evidence(
                card,
                question_source_id=SOURCE_IDS["paper_answer"],
    ...
    for number in range(41, 46):
        ...
        card["explanation"] = "答案已按本地百度网盘 2020 英语二答案速查表匹配；Part B 按人名与观点 A-G 信息匹配训练。"
        cards.append(
            attach_evidence(
                card,
                question_source_id=SOURCE_IDS["paper_answer"],
    ...
    translation["explanation"] = "参考译文已按本地百度网盘 2020 英语二答案速查 PDF 匹配，并与逐题细解 PDF 英汉对照复核。"
    cards.append(
        attach_evidence(
            translation,
            question_source_id=SOURCE_IDS["paper_answer"],
    ...
    return {
        ...
        "sourceFiles": [
            {
                "id": "english2-2020-document-version",
            ...
            {
                "id": "english2-2020-original-paper",
            ...
            {
                "id": "english2-2020-detailed-analysis",
...
def validate_cards(cards: list[dict[str, Any]], material: dict[str, Any], text: str, detail_text: str) -> None:
    ...
    if "Being a good parent" not in material["cloze_passage"] or "patient parenting" not in material["cloze_passage"]:
        issues.append("cloze passage missing parenting anchors")
    for number in range(1, 21):
        if f"({number})" not in material["cloze_passage"]:
            issues.append(f"cloze passage missing blank marker ({number})")
    if "robotic rats" not in material["passages"]["text1"] or "Laleh Quinn" not in material["passages"]["text1"]:
        issues.append("Text 1 passage missing rats/Quinn anchors")
    if "CEO pay" not in material["passages"]["text2"] or "$18.9 million" not in material["passages"]["text2"]:
        issues.append("Text 2 passage missing CEO pay anchors")
    if "Madrid" not in material["passages"]["text3"] or "ultra-low emission zone" not in material["passages"]["text3"]:
        issues.append("Text 3 passage missing Madrid/Ulez anchors")
    if "Generation Z" not in material["passages"]["text4"] or "$1.5 trillion" not in material["passages"]["text4"]:
        issues.append("Text 4 passage missing Gen Z/debt anchors")
    if "Five Ways to Win Over Everyone in the Office" not in material["passages"]["part-b"] or "Harvard Business School" not in material["passages"]["part-b"]:
        issues.append("Part B passage missing office-interaction anchors")
    if "Tailor your interactions" not in json.dumps(PART_B_OPTIONS):
        issues.append("Part B options missing office subheading anchors")
    if "experiencing some kind of failure" not in material["translation_text"] or "unexpected motivation to succeed" not in material["translation_text"]:
        issues.append("translation text missing failure/motivation anchors")
    if "英语（二）试题" in material["translation_text"]:
        issues.append("translation text contains PDF page footer")
    if "某高校学生手机阅读目的调查" not in material["writing_prompts"][48] or "59.5%" not in material["writing_prompts"][48]:
        issues.append("writing prompt 48 missing mobile-reading chart anchor")
    if "BCADD" not in text or "EFDAC" not in text:
