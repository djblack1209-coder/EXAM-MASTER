#!/usr/bin/env python3
"""
Build the 2014 Math III public-course bank from the local Baidu Netdisk PDF.
...
DEFAULT_OUTPUT = PROJECT_ROOT / "src" / "config" / "flashcard-banks" / "math3-2014.json"
DEFAULT_ASSET_DIR = PROJECT_ROOT / "cdn-assets" / "question-bank" / "math3-2014"
TMP_RENDER_DIR = PROJECT_ROOT / "tmp" / "pdfs" / "math3-2014-release-assets"

PAPER_ANSWER = RAW_DIR / "src_bd3bb496ed9148e3cd0127d9-2014年考研数学三真题及解析.pdf"

SOURCE_IDS = {
    "paper_answer": "math3-2014-paper-answer",
}

PAPER_SOURCE_ID = "src_bd3bb496ed9148e3cd0127d9"
...
SHORT_ANSWERS = {
    15: "极限为 1/2。",
    16: "二重积分值为 -3/4。",
    17: "f(u) = -u/4 - 1/16 + e^(4u)/16。",
    18: "幂级数收敛域及和函数见答案原页。",
    19: "证明过程与结论见答案原页。",
    20: "a=-1；方程组基础解系及相关矩阵结论见答案原页。",
    21: "a=1；合同变换及标准形见答案原页。",
    22: "分布函数、密度函数及相关概率计算见答案原页。",
    23: "参数估计量及概率分布计算见答案原页。",
}
...
def question_image_ref(crop_id: str) -> dict[str, str]:
    return {
        "src": f"question-bank/math3-2014/question-{crop_id}.jpg",
        "caption": f"题面裁切图 q{crop_id}",
        "alt": f"2014考研数学三第{crop_id.split('-')[0]}题题面裁切图",
    }


def answer_image_ref(page: int) -> dict[str, str]:
    return {
        "src": f"question-bank/math3-2014/answer-page-{page:02d}.jpg",
        "caption": f"答案解析 p.{page}",
        "alt": f"2014考研数学三答案解析 p.{page}",
    }
...
def question_anchor(number: int) -> str:
    return f"2014考研数学三第{number}题（{section_for(number)}）。题干、公式和图形以题面裁切图为准。"


def explanation_for(number: int) -> str:
    if number <= 8:
        return "答案来自本地百度网盘 2014 数学三真题及解析 PDF；选择题解析见答案原页图。"
    if number <= 14:
        return "答案来自本地百度网盘 2014 数学三真题及解析 PDF；完整题干、公式和解析见原页图。"
    return "答案来自本地百度网盘 2014 数学三真题及解析 PDF；完整演算、证明或推导过程见答案原页图。"


def source_evidence(number: int, answer_hash: str) -> dict[str, Any]:
    return {
        "evidenceId": f"math3-2014-q{number:02d}",
...
def build_card(number: int) -> dict[str, Any]:
    ...
    return {
        "id": f"math3-2014-{number:03d}",
        "number": number,
        "paperId": "math3-2014",
        "paperName": "2014考研数学三真题",
        "subject": "数学",
        "subjectKey": "math",
        "track": "math3",
        "year": "2014",
        ...
        "tags": ["math3", "2014", section_for(number)],
...
def validate_cards(cards: list[dict[str, Any]], *, asset_dir: Path) -> None:
    ...
    for expected_number, card in enumerate(cards, start=1):
        ...
        for image in [*card.get("questionImages", []), *card.get("answerImages", [])]:
            relative = str(image.get("src", "")).removeprefix("question-bank/math3-2014/")
    ...
    if issues:
        raise ValueError("math3-2014 validation failed:\n" + "\n".join(issues))
...
def build_payload(*, asset_dir: Path, force_assets: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "assetRoot": "question-bank/math3-2014",
