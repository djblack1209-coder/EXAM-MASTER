#!/usr/bin/env python3
"""
Build the 2009 Math II public-course bank from the local Baidu Netdisk PDF.
...
RAW_DIR = PROJECT_ROOT / "data" / "raw-inbox" / "public-course-history" / "math2" / "2009"
DEFAULT_OUTPUT = PROJECT_ROOT / "src" / "config" / "flashcard-banks" / "math2-2009.json"
DEFAULT_ASSET_DIR = PROJECT_ROOT / "cdn-assets" / "question-bank" / "math2-2009"
TMP_RENDER_DIR = PROJECT_ROOT / "tmp" / "pdfs" / "math2-2009-release-assets"

PAPER_ANSWER = RAW_DIR / "2009-math2-paper-answer.pdf"

SOURCE_IDS = {
    "paper_answer": "math2-2009-paper-answer",
}

PAPER_SOURCE_ID = "src_e68710e55d77aec12d161598"
...
SHORT_ANSWERS = {
    15: "极限值为 1/4。",
    16: "不定积分的原函数表达式见答案原页。",
    17: "全微分与二阶混合偏导表达式见答案原页。",
    18: "旋转体体积为 17π/6；通解与面积条件推导见答案原页。",
    19: "二重积分值为 -8/3。",
    20: "分段函数 y(x) 的表达式见答案原页。",
    21: "拉格朗日中值定理证明及 f'(0)=A 的证明见答案原页。",
    22: "两个线性方程组的解向量与线性无关证明见答案原页。",
    23: "二次型矩阵特征值为 a、a-2、a+1；规范形条件下 a=2。",
}
...
def image_ref(kind: str, page: int, *, caption: str) -> dict[str, str]:
    ...
    return {
        "src": f"question-bank/math2-2009/{file_name}",
        "caption": caption,
        "alt": f"2009考研数学二{caption}",
    }
...
def question_anchor(number: int) -> str:
    return f"2009考研数学二第{number}题（{section_for(number)}）。题干、公式和图形以试卷原页图为准。"


def explanation_for(number: int) -> str:
    if number <= 8:
        return "答案来自本地百度网盘 2009 数学二真题及解析 PDF；选择题解析见答案原页图。"
    if number <= 14:
        return "答案来自本地百度网盘 2009 数学二真题及解析 PDF；完整题干和公式见试卷原页图。"
    return "答案来自本地百度网盘 2009 数学二真题及解析 PDF；完整演算、证明或推导过程见答案原页图。"


def source_evidence(number: int, answer_hash: str) -> dict[str, Any]:
    return {
        "evidenceId": f"math2-2009-q{number:02d}",
...
def build_card(number: int) -> dict[str, Any]:
    ...
    return {
        "id": f"math2-2009-{number:03d}",
        "number": number,
        "paperId": "math2-2009",
        "paperName": "2009考研数学二真题",
        "subject": "数学",
        "subjectKey": "math",
        "track": "math2",
        "year": "2009",
        ...
        "tags": ["math2", "2009", section_for(number)],
...
def build_payload(*, asset_dir: Path, force_assets: bool) -> dict[str, Any]:
    render_pdf_assets(PAPER_ANSWER, "paper-page", asset_dir=asset_dir, force=force_assets, pages=set(range(1, 19)))
    render_pdf_assets(PAPER_ANSWER, "answer-page", asset_dir=asset_dir, force=force_assets, pages=set(range(1, 19)))
    ...
    return {
        ...
        "assetRoot": "question-bank/math2-2009",
