#!/usr/bin/env python3
"""
Build the 2020 Politics public-course bank from local Baidu Netdisk PDFs.

The standalone paper PDF has a usable text layer and defines the user-visible
question pages. The paper-answer PDF is scanned, so release evidence stays as
rendered PDF page images; OCR is used only to locate answers and answer pages.
"""
...
SOURCE_ANSWER_PDF = (
    ...
    / "politics"
    / "2020"
    / "2020-politics-paper-answer.pdf"
)
DEFAULT_OUTPUT = PROJECT_ROOT / "src" / "config" / "flashcard-banks" / "politics-2020.json"
DEFAULT_ASSET_DIR = PROJECT_ROOT / "cdn-assets" / "question-bank" / "politics-2020"
TMP_RENDER_DIR = PROJECT_ROOT / "tmp" / "pdfs" / "politics-2020-release-assets"

PAPER_ID = "politics-2020"
PAPER_SOURCE_EVIDENCE_ID = "politics-2020-paper"
ANSWER_SOURCE_EVIDENCE_ID = "politics-2020-paper-answer"
PAPER_SOURCE_MANIFEST_ID = "src_06315e7fedd219c2b5075ba6"
ANSWER_SOURCE_MANIFEST_ID = "src_c5281046ec5e3b17d92a4143"
...
ANALYSIS_ANSWERS = {
    ...
    37: "分析题参考答案要点见答案原页 p.15-p.16。",
    38: "分析题参考答案要点见答案原页 p.16。",
}

QUESTION_PAGES = {
    **{number: [1] for number in range(1, 5)},
    **{number: [2] for number in range(5, 9)},
    **{number: [3] for number in range(9, 14)},
    **{number: [4] for number in range(14, 18)},
    **{number: [5] for number in range(18, 21)},
    **{number: [6] for number in range(21, 25)},
    **{number: [7] for number in range(25, 30)},
    **{number: [8] for number in range(30, 34)},
    34: [9, 10],
    35: [10, 11],
    36: [11, 12],
    37: [12, 13],
...
ANSWER_PAGES = {
    **{number: [2] for number in range(1, 4)},
    **{number: [3] for number in range(4, 7)},
    **{number: [4] for number in range(7, 10)},
    **{number: [5] for number in range(10, 13)},
    **{number: [6] for number in range(13, 16)},
    16: [7],
    17: [7],
    18: [7, 8],
    **{number: [8] for number in range(19, 22)},
    **{number: [9] for number in range(22, 24)},
    24: [9, 10],
    **{number: [10] for number in range(25, 27)},
    **{number: [11] for number in range(27, 30)},
    **{number: [12] for number in range(30, 33)},
    33: [12, 13],
    34: [13],
    35: [13, 14],
    36: [14, 15],
...
def render_all_assets(*, asset_dir: Path, force: bool) -> None:
    render_pdf_assets(SOURCE_PAPER_PDF, asset_dir=asset_dir, prefix="paper-page", expected_pages=14, force=force)
    render_pdf_assets(SOURCE_ANSWER_PDF, asset_dir=asset_dir, prefix="answer-page", expected_pages=16, force=force)


def image_ref(page: int, *, prefix: str, caption: str) -> dict[str, str]:
    return {
        "src": f"question-bank/politics-2020/{prefix}-{page:02d}.jpg",
        "caption": caption,
        "alt": f"2020考研政治{caption}",
    }
...
def question_anchor(number: int) -> str:
    ...
    return f"2020考研政治第{number}题（{section}）。题干、选项和材料以试卷原页图为准。"


def explanation_for(number: int) -> str:
    if number <= 33:
        return (
            "答案来自本地百度网盘 2020 考研政治真题及参考答案 PDF 第 2-13 页逐题解析；"
            "题库以独立试卷 PDF 作为用户可见题面和选项顺序。"
        )
    return "参考答案来自本地百度网盘 2020 考研政治真题及参考答案 PDF；完整答案要点见答案原页图。"


def evidence(number: int, question_hash: str, answer_hash: str) -> dict[str, str]:
    return {
        "evidenceId": f"politics-2020-q{number:02d}",
...
def answer_evidence(number: int, answer_hash: str) -> dict[str, str]:
    return {
        ...
        "verifiedBy": "build_politics_2020_bank",
        "note": "Matched against local Baidu Netdisk 2020 Politics paper-answer PDF page images.",
    }
...
def build_card(number: int) -> dict[str, Any]:
    ...
    return {
        ...
        "tags": ["politics", "2020", section_for(number)],
...
def validate_cards(cards: list[dict[str, Any]]) -> None:
    ...
    if issues:
        raise ValueError("politics-2020 validation failed:\n" + "\n".join(issues))
...
def build_payload(*, asset_dir: Path, force_assets: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "sourceNotes": [
            "The 2020 standalone paper PDF provides the release question pages and user-visible option order. The scanned paper-answer PDF contains per-question choice answers on pages 2-13 and analysis answer points on pages 13-16; OCR was used only to locate those pages while final release evidence remains rendered PDF page images.",
        ],
        "assetRoot": "question-bank/politics-2020",
