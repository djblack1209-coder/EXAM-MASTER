#!/usr/bin/env python3
"""
Build the 2010 Politics public-course bank from local Baidu Netdisk PDFs.

The 2010 source set needs two files: the standalone paper PDF has a usable
text layer for most question pages, while the paper-answer PDF has the full
answer explanations and includes the missing question 21 wording.
"""
...
SOURCE_ANSWER_PDF = (
    ...
    / "politics"
    / "2010"
    / "2010-politics-paper-answer.pdf"
)
DEFAULT_OUTPUT = PROJECT_ROOT / "src" / "config" / "flashcard-banks" / "politics-2010.json"
DEFAULT_ASSET_DIR = PROJECT_ROOT / "cdn-assets" / "question-bank" / "politics-2010"
TMP_RENDER_DIR = PROJECT_ROOT / "tmp" / "pdfs" / "politics-2010-release-assets"

PAPER_ID = "politics-2010"
PAPER_SOURCE_EVIDENCE_ID = "politics-2010-paper"
ANSWER_SOURCE_EVIDENCE_ID = "politics-2010-paper-answer"
PAPER_SOURCE_MANIFEST_ID = "src_095ea7cbf62c381b2ecdadf4"
ANSWER_SOURCE_MANIFEST_ID = "src_89287710a63711bcbb6f0c10"
...
ANALYSIS_ANSWERS = {
    34: "分析题参考答案要点见答案原页 p.22-p.23。",
    35: "分析题参考答案要点见答案原页 p.23-p.24。",
    36: "分析题参考答案要点见答案原页 p.24-p.25。",
    37: "分析题参考答案要点见答案原页 p.25-p.26。",
    38: "分析题参考答案要点见答案原页 p.26-p.27。",
}

QUESTION_PAGES = {
    **{number: [1] for number in range(1, 8)},
    **{number: [2] for number in range(8, 17)},
...
ANSWER_PAGES = {
    **{number: [13] for number in range(1, 5)},
    **{number: [14] for number in range(5, 9)},
    **{number: [15] for number in range(9, 13)},
    **{number: [16] for number in range(13, 17)},
    17: [17],
    18: [17],
    19: [17],
    20: [17, 18],
...
QUESTION_21 = {
    "question": "中国革命建设和改革的实践证明，要运用马克思主义指导实践，必须实现马克思主义中国化，马克思主义之所以能够中国化的原因在于：",
    "options": [
        {"label": "A", "text": "马克思主义理论的内在要求"},
        {"label": "B", "text": "马克思主义与中华民族优秀文化具有相融性"},
        {"label": "C", "text": "中国革命建设和改革的实践需要马克思主义指导"},
        {"label": "D", "text": "马克思主义为中国革命建设和改革提供了现实发展模式"},
    ],
}
...
def render_all_assets(*, asset_dir: Path, force: bool) -> None:
    ...
    render_pdf_assets(SOURCE_ANSWER_PDF, asset_dir=asset_dir, prefix="answer-page", expected_pages=27, force=force)


def image_ref(page: int, *, prefix: str, caption: str) -> dict[str, str]:
    return {
        "src": f"question-bank/politics-2010/{prefix}-{page:02d}.jpg",
        "caption": caption,
        "alt": f"2010考研政治{caption}",
    }
...
def question_anchor(number: int) -> str:
    if number == 21:
        return QUESTION_21["question"]
    section = section_for(number)
    return f"2010考研政治第{number}题（{section}）。题干、选项和材料以试卷原页图为准。"


def explanation_for(number: int) -> str:
    if number <= 33:
        return "答案来自本地百度网盘 2010 考研政治真题及参考答案 PDF 的逐题解析页；题面以真题 PDF 原页图为准。"
    return "参考答案来自本地百度网盘 2010 考研政治真题及参考答案 PDF；完整答案要点见答案原页图。"


def evidence(number: int, question_hash: str, answer_hash: str) -> dict[str, str]:
    return {
        "evidenceId": f"politics-2010-q{number:02d}",
        "sourceId": PAPER_SOURCE_MANIFEST_ID if number != 21 else ANSWER_SOURCE_MANIFEST_ID,
...
def answer_evidence(number: int, answer_hash: str) -> dict[str, str]:
    return {
        ...
        "verifiedBy": "build_politics_2010_bank",
        "note": "Matched against local Baidu Netdisk 2010 Politics paper-answer PDF.",
    }
...
def build_card(number: int) -> dict[str, Any]:
    ...
    question_pages = QUESTION_PAGES.get(number, [])
    question_images = [
        image_ref(page, prefix="paper-page", caption=f"试卷原页 p.{page}") for page in question_pages
    ]
    answer_images = [
        image_ref(page, prefix="answer-page", caption=f"答案原页 p.{page}") for page in ANSWER_PAGES[number]
    ]
    ...
    return {
        ...
        "tags": ["politics", "2010", section_for(number)],
...
def validate_cards(cards: list[dict[str, Any]]) -> None:
    ...
    for expected_number, card in enumerate(cards, start=1):
        ...
        if expected_number != 21 and not card.get("questionImages"):
    ...
    if issues:
        raise ValueError("politics-2010 validation failed:\n" + "\n".join(issues))
...
def build_payload(*, asset_dir: Path, force_assets: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "sourceNotes": [
            "Standalone 2010 paper PDF omits question 21; question 21 wording is recovered from the paper-answer PDF and rendered as text without a question image to avoid exposing the answer before submission."
        ],
        "assetRoot": "question-bank/politics-2010",
