#!/usr/bin/env python3
"""
Build the 2005 Politics public-course bank from a local Baidu Netdisk PDF.

The source PDF is image-based. OCR is sufficient to confirm the answer key, but
not reliable enough for full question text, so the generated bank uses concise
anchors plus rendered PDF page images as the authoritative question/answer
material.
"""
...
SOURCE_PDF = (
    ...
    / "politics"
    / "2005"
    / "2005-politics-paper-answer.pdf"
)
DEFAULT_OUTPUT = PROJECT_ROOT / "src" / "config" / "flashcard-banks" / "politics-2005.json"
DEFAULT_ASSET_DIR = PROJECT_ROOT / "cdn-assets" / "question-bank" / "politics-2005"
TMP_RENDER_DIR = PROJECT_ROOT / "tmp" / "pdfs" / "politics-2005-release-assets"

PAPER_ID = "politics-2005"
SOURCE_EVIDENCE_ID = "politics-2005-paper-answer"
SOURCE_MANIFEST_ID = "src_e89f5c81ca06f7f38ebf4b4d"
...
ANALYSIS_ANSWERS = {
    31: "辨析题参考答案要点见答案原页 p.9。",
    32: "辨析题参考答案要点见答案原页 p.9。",
    33: "辨析题参考答案要点见答案原页 p.9-p.10。",
    34: "分析题参考答案要点见答案原页 p.10。",
    35: "分析题参考答案要点见答案原页 p.10。",
    36: "分析题参考答案要点见答案原页 p.10-p.11。",
    37: "选做分析题参考答案要点见答案原页 p.11-p.12。",
}

QUESTION_PAGES = {
    **{number: [1] for number in range(1, 8)},
    **{number: [2] for number in range(8, 16)},
    **{number: [3] for number in range(16, 23)},
    **{number: [4] for number in range(23, 29)},
    29: [4, 5],
    30: [5],
    31: [5],
    32: [5],
...
ANSWER_PAGES = {
    **{number: [9] for number in range(1, 33)},
    33: [9, 10],
    34: [10],
    35: [10],
    36: [10, 11],
...
def image_ref(page: int, *, caption: str) -> dict[str, str]:
    return {
        "src": f"question-bank/politics-2005/paper-page-{page:02d}.jpg",
        "caption": caption,
        "alt": f"2005考研政治{caption}",
    }
...
def question_anchor(number: int) -> str:
    ...
    return f"2005考研政治第{number}题（{section}）。题干、选项和材料以试卷原页图为准。"


def explanation_for(number: int) -> str:
    if number <= 30:
        return "答案来自本地百度网盘 2005 考研政治试题及参考答案 PDF；完整题干和选项见试卷原页图。"
    return "参考答案来自本地百度网盘 2005 考研政治试题及参考答案 PDF；完整答案要点见答案原页图。"


def evidence(number: int, question_hash: str, answer_hash: str) -> dict[str, str]:
    return {
        "evidenceId": f"politics-2005-q{number:02d}",
...
def answer_evidence(number: int, answer_hash: str) -> dict[str, str]:
    return {
        "status": "matched",
        "evidenceRole": "official_answer_key" if number <= 30 else "official_reference_answer",
        "sourceId": SOURCE_MANIFEST_ID,
        "sourceFilePath": str(SOURCE_PDF.relative_to(PROJECT_ROOT)),
        "answerTextHash": answer_hash,
        "method": "local_pdf_ocr_confirmed_page_image",
        "verifiedBy": "build_politics_2005_bank",
        "note": "Matched against local Baidu Netdisk 2005 Politics paper-answer PDF.",
    }
...
def build_card(number: int) -> dict[str, Any]:
    ...
    answer_images = [
        image_ref(page, caption=f"答案原页 p.{page}") for page in ANSWER_PAGES[number]
    ]
    ...
    return {
        ...
        "tags": ["politics", "2005", section_for(number)],
        "difficulty": 3 if number > 30 else 2,
...
def validate_cards(cards: list[dict[str, Any]]) -> None:
    ...
    expected_counts = {"single_choice": 15, "multi_choice": 15, "analysis": 7}
    if len(cards) != 37:
        issues.append(f"card count {len(cards)} != 37")
    if counts != expected_counts:
        issues.append(f"type counts {counts} != {expected_counts}")
    if issues:
        raise ValueError("politics-2005 validation failed:\n" + "\n".join(issues))
...
def build_payload(*, asset_dir: Path, force_assets: bool) -> dict[str, Any]:
    ...
    cards = [build_card(number) for number in range(1, 38)]
    ...
    return {
        ...
        "assetRoot": "question-bank/politics-2005",
        "processed_at": utc_now(),
        "total_cards": len(cards),
        "sections": ["单项选择", "多项选择", "辨析题", "分析题"],
        "cards": cards,
    }
