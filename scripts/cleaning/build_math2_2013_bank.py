#!/usr/bin/env python3
"""
Build the 2013 Math II public-course bank from the local Baidu Netdisk PDF.
...
RAW_DIR = PROJECT_ROOT / "data" / "raw-inbox" / "public-course-history" / "math2" / "2013"
DEFAULT_OUTPUT = PROJECT_ROOT / "src" / "config" / "flashcard-banks" / "math2-2013.json"
DEFAULT_ASSET_DIR = PROJECT_ROOT / "cdn-assets" / "question-bank" / "math2-2013"
TMP_RENDER_DIR = PROJECT_ROOT / "tmp" / "pdfs" / "math2-2013-release-assets"

PAPER_ANSWER = RAW_DIR / "2013-math2-paper-answer.pdf"

SOURCE_IDS = {
    "paper_answer": "math2-2013-paper-answer",
}

PAPER_SOURCE_ID = "src_e6637ec0e4abce970144c5a1"
...
FILL_ANSWERS = {
    ...
    10: "1 / sqrt(1 - e^(-1))",
    11: "pi/12",
    12: "y + x - pi/4 - ln(sqrt(2)) = 0",
    13: "y = e^(3x) - e^x - x e^(2x)",
    14: "-1",
}
...
SHORT_ANSWERS = {
    ...
    17: "二重积分值为 416/3。",
    18: "证明结论见答案原页。",
    19: "最长距离为 sqrt(2)，最短距离为 1。",
    20: "f(x) 的最小值为 1；数列极限为 1。",
    21: "曲线弧长与形心横坐标见答案原页。",
    22: "a=-1，b=0；全部矩阵 C 见答案原页。",
    23: "标准形为 2y1^2 + y2^2；证明见答案原页。",
}

QUESTION_PAGES = {
    **{number: [1] for number in range(1, 8)},
    8: [1, 2],
    **{number: [2] for number in range(9, 17)},
...
def image_ref(kind: str, page: int, *, caption: str) -> dict[str, str]:
    ...
    return {
        "src": f"question-bank/math2-2013/{file_name}",
        "caption": caption,
        "alt": f"2013考研数学二{caption}",
    }
...
def question_anchor(number: int) -> str:
    return f"2013考研数学二第{number}题（{section_for(number)}）。题干、公式和图形以试卷原页图为准。"


def explanation_for(number: int) -> str:
    if number <= 8:
        return "答案来自本地百度网盘 2013 数学二真题及解析 PDF；选择题解析见答案原页图。"
    if number <= 14:
        return "答案来自本地百度网盘 2013 数学二真题及解析 PDF；完整题干和公式见试卷原页图。"
    return "答案来自本地百度网盘 2013 数学二真题及解析 PDF；完整演算、证明或推导过程见答案原页图。"


def source_evidence(number: int, answer_hash: str) -> dict[str, Any]:
    return {
        "evidenceId": f"math2-2013-q{number:02d}",
...
def build_card(number: int) -> dict[str, Any]:
    ...
    return {
        "id": f"math2-2013-{number:03d}",
        "number": number,
        "paperId": "math2-2013",
        "paperName": "2013考研数学二真题",
        "subject": "数学",
        "subjectKey": "math",
        "track": "math2",
        "year": "2013",
        ...
        "tags": ["math2", "2013", section_for(number)],
...
def build_payload(*, asset_dir: Path, force_assets: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "assetRoot": "question-bank/math2-2013",
