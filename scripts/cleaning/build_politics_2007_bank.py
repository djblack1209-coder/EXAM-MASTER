#!/usr/bin/env python3
"""
Build the 2007 Politics public-course bank from a local Baidu Netdisk PDF.

The source PDF is image-based and includes inline choice answers plus reference
answer sections for analysis questions. The generated bank keeps rendered PDF
pages as the authoritative question/answer material.
"""
...
SOURCE_PDF = (
    ...
    / "politics"
    / "2007"
    / "2007-politics-paper-answer.pdf"
)
DEFAULT_OUTPUT = PROJECT_ROOT / "src" / "config" / "flashcard-banks" / "politics-2007.json"
DEFAULT_ASSET_DIR = PROJECT_ROOT / "cdn-assets" / "question-bank" / "politics-2007"
TMP_RENDER_DIR = PROJECT_ROOT / "tmp" / "pdfs" / "politics-2007-release-assets"

PAPER_ID = "politics-2007"
SOURCE_EVIDENCE_ID = "politics-2007-paper-answer"
SOURCE_MANIFEST_ID = "src_95c639bb87c2501fb7690c9f"
...
ANALYSIS_ANSWERS = {
    ...
    35: "分析题参考答案要点见答案原页 p.12。",
    36: "分析题参考答案要点见答案原页 p.13-p.14。",
    37: "分析题参考答案要点见答案原页 p.15-p.16。",
    38: "选做分析题参考答案要点见答案原页 p.18-p.20。",
}
...
def render_pdf_assets(pdf: Path, *, asset_dir: Path, force: bool) -> None:
    ...
    if len(expected) >= 20 and not force:
        return
...
def image_ref(page: int, *, caption: str) -> dict[str, str]:
    return {
        "src": f"question-bank/politics-2007/paper-page-{page:02d}.jpg",
        "caption": caption,
        "alt": f"2007考研政治{caption}",
    }
...
def question_anchor(number: int) -> str:
    ...
    return f"2007考研政治第{number}题（{section}）。题干、选项和材料以试卷原页图为准。"


def explanation_for(number: int) -> str:
    if number <= 33:
        return "答案来自本地百度网盘 2007 考研政治真题及参考答案 PDF 内嵌答案；完整题干和选项见试卷原页图。"
    return "参考答案来自本地百度网盘 2007 考研政治真题及参考答案 PDF；完整答案要点见答案原页图。"


def evidence(number: int, question_hash: str, answer_hash: str) -> dict[str, str]:
    return {
        "evidenceId": f"politics-2007-q{number:02d}",
...
def answer_evidence(number: int, answer_hash: str) -> dict[str, str]:
    return {
        ...
        "verifiedBy": "build_politics_2007_bank",
        "note": "Matched against local Baidu Netdisk 2007 Politics paper-answer PDF.",
    }
...
def build_card(number: int) -> dict[str, Any]:
    ...
    return {
        ...
        "tags": ["politics", "2007", section_for(number)],
...
def validate_cards(cards: list[dict[str, Any]]) -> None:
    ...
    if issues:
        raise ValueError("politics-2007 validation failed:\n" + "\n".join(issues))
...
def build_payload(*, asset_dir: Path, force_assets: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "assetRoot": "question-bank/politics-2007",
