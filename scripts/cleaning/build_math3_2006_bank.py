#!/usr/bin/env python3
"""
Build the 2006 Math III public-course bank from the local Baidu Netdisk PDF.
...
evidence; OCR is only used during operator audit to confirm answer keys and
page anchors.
"""
...
RAW_DIR = PROJECT_ROOT / "data" / "raw-inbox" / "public-course-history" / "math3" / "2006"
DEFAULT_OUTPUT = PROJECT_ROOT / "src" / "config" / "flashcard-banks" / "math3-2006.json"
DEFAULT_ASSET_DIR = PROJECT_ROOT / "cdn-assets" / "question-bank" / "math3-2006"
TMP_RENDER_DIR = PROJECT_ROOT / "tmp" / "pdfs" / "math3-2006-release-assets"

PAPER_ANSWER = RAW_DIR / "2006-math3-paper-answer.pdf"

SOURCE_IDS = {
    "paper_answer": "math3-2006-paper-answer",
}

PAPER_SOURCE_ID = "src_cbfa1c8608ea693dcf2fdde1"
...
SHORT_ANSWERS = {
    ...
    16: "2/5",
    17: "完整不等式证明见答案原页。",
    18: "曲线 L: y = ax^2 - ax；a = 2。",
    19: "S(x) = 2x^2 arctan x - x ln(1 + x^2)，x in [-1,1]。",
    20: "a = 0 或 a = -10；极大线性无关组及线性表示见答案原页。",
    21: "A 的特征值为 3,0,0；正交矩阵、对角矩阵和后续计算见答案原页。",
    22: "Y 的概率密度、Cov(X,Y)=2/3 和分布函数取值见答案原页。",
    23: "theta 的矩估计为 3/2 - Xbar；最大似然估计为 N/n。",
}

QUESTION_PAGES = {
    **{number: [1] for number in range(1, 9)},
    9: [1, 2],
    **{number: [2] for number in range(10, 14)},
    14: [2, 3],
...
def image_ref(kind: str, page: int, *, caption: str) -> dict[str, str]:
    ...
    return {
        "src": f"question-bank/math3-2006/{file_name}",
        "caption": caption,
        "alt": f"2006考研数学三{caption}",
    }
...
def question_anchor(number: int) -> str:
    return f"2006考研数学三第{number}题（{section_for(number)}）。题干、公式和图形以试卷原页图为准。"


def explanation_for(number: int) -> str:
    if number <= 6:
        return "答案来自本地百度网盘 2006 数学三真题及解析 PDF；完整题干和公式见试卷原页图。"
    if number <= 14:
        return "答案来自本地百度网盘 2006 数学三真题及解析 PDF；选择题解析见答案原页图。"
    return "答案来自本地百度网盘 2006 数学三真题及解析 PDF；完整演算、证明或推导过程见答案原页图。"


def source_evidence(number: int, answer_hash: str) -> dict[str, Any]:
    return {
        "evidenceId": f"math3-2006-q{number:02d}",
...
def build_card(number: int) -> dict[str, Any]:
    ...
    return {
        "id": f"math3-2006-{number:03d}",
        "number": number,
        "paperId": "math3-2006",
        "paperName": "2006考研数学三真题",
        "subject": "数学",
        "subjectKey": "math",
        "track": "math3",
        "year": "2006",
        ...
        "tags": ["math3", "2006", section_for(number)],
...
def build_payload(*, asset_dir: Path, force_assets: bool) -> dict[str, Any]:
    ...
    render_pdf_assets(PAPER_ANSWER, "answer-page", asset_dir=asset_dir, force=force_assets, pages=set(range(5, 20)))
    ...
    return {
        ...
        "assetRoot": "question-bank/math3-2006",
