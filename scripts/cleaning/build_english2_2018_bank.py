#!/usr/bin/env python3
"""Build the 2018 English II public-course flashcard bank."""
...
DEFAULT_OUTPUT = PROJECT_ROOT / "src" / "config" / "flashcard-banks" / "english2-2018.json"

DOCUMENT_VERSION = RAW_DIR / "src_bb76d07247dcb5e9691b37dc-2018.pdf"
ORIGINAL_PAPER = RAW_DIR / "src_d94f09a02cc4c2e33c83c874-2018年考研英语二真题.pdf"
PAPER_ANSWER = RAW_DIR / "src_304822354811c024c7644fe7-2018年真题及答案速查.pdf"
DETAILED_ANALYSIS = RAW_DIR / "src_8ca8d55ed290ecfc593c4aef-2018年真题逐题细解.pdf"

SOURCE_IDS = {
    "document_version": "src_bb76d07247dcb5e9691b37dc",
    "original_paper": "src_d94f09a02cc4c2e33c83c874",
    "paper_answer": "src_304822354811c024c7644fe7",
    "detailed_analysis": "src_8ca8d55ed290ecfc593c4aef",
}

CHOICE_ANSWERS = {
    **dict(zip(range(1, 6), "DBCBA")),
    **dict(zip(range(6, 11), "BCDAC")),
    **dict(zip(range(11, 16), "BCADD")),
    **dict(zip(range(16, 21), "DABDC")),
    **dict(zip(range(21, 26), "BDACC")),
    **dict(zip(range(26, 31), "CAACC")),
    **dict(zip(range(31, 36), "BCDAD")),
    **dict(zip(range(36, 41), "ADDBB")),
    **dict(zip(range(41, 46), "AFEBD")),
}
...
WRITING_CHART_48 = (
    "Chart: 2017年某市消费者选择餐厅时的关注因素（特色36.3%；服务26.8%；"
    "环境23.8%；价格8.4%；其他4.7%）."
)
...
def require_sources() -> None:
    ...
    if missing:
        raise FileNotFoundError("Missing required English II 2018 source PDFs: " + ", ".join(missing))
...
def extract_cloze_passage(text: str) -> str:
    ...
    block = re.sub(r"need to\s+\]\s+uncertainty", "need to (1) uncertainty", block, count=1)
...
def extract_passages(text: str) -> dict[str, str]:
    passages = {
        "text1": cleaned_block(text_between(text, "Text 1", "21. A broken bike chain")),
        "text2": cleaned_block(text_between(text, "Text 2", "26. The word")),
        "text3": cleaned_block(text_between(text, "Text 3", "31. According to Paragraph 1")),
        "text4": cleaned_block(text_between(text, "Text 4", "36. The key to mastering")),
        "part-b": cleaned_block(text_between(text, "Five ways to make conversation with anyone", "Section ID")),
    }
    passages["part-b"] = f"Five ways to make conversation with anyone {passages['part-b']}"
    return passages
...
def extract_translation_text(text: str) -> str:
    block = text_between(text, "A fifth grader gets a homework assignment", "Section IV")
    return compact(page_footer_pattern().sub("", f"A fifth grader gets a homework assignment{block}"))


def extract_translation_answer(text: str) -> str:
    block = text_between(text, "一个五年级学生的一项家庭作业是", "英语（二）试题.15.（共15页）")
    return cleaned_block("一个五年级学生的一项家庭作业是" + block)


def extract_writing_prompts(text: str) -> dict[int, str]:
    prompt47 = cleaned_block(text_between(text, "47. Directions:", "Part B"))
    prompt48 = cleaned_block(text_between(text, "48. Directions:", "2018年考研英语（二）真题答案速查表"))
...
def attach_evidence(
    ...
    *,
    ...
    card["sourceEvidence"] = {
        ...
        "verifiedAt": now,
        "verifiedBy": "build_english2_2018_bank",
    }
...
def base_card(number: int, *, card_type: str, section: str, group_id: str, question: str, answer: str) -> dict[str, Any]:
    return {
        "id": f"english2-2018-{number:03d}",
        "paperId": "english2-2018",
        "paperName": "2018考研英语二真题",
        "subject": "英语",
        "subjectKey": "english",
        "track": "english2",
        "year": "2018",
...
def build_bank(now: str | None = None) -> dict[str, Any]:
    ...
    for number in range(1, 21):
        ...
        card["explanation"] = "答案已按本地百度网盘 2018 英语二真题答案速查表匹配，并用逐题细解 PDF 复核。"
        cards.append(
            attach_evidence(
                card,
                ...
                answer_note="Matched against the local 2018 English II answer-speed PDF.",
            )
        )
    ...
    for number in range(21, 41):
        ...
        card["explanation"] = "答案已按本地百度网盘 2018 英语二答案速查表匹配，并用逐题细解 PDF 阅读解析复核。"
        cards.append(
            attach_evidence(
                card,
                question_source_id=SOURCE_IDS["paper_answer"],
    ...
    for number in range(41, 46):
        ...
        card["explanation"] = "答案已按本地百度网盘 2018 英语二答案速查表匹配；Part B 按 A-G 小标题选择题训练。"
        cards.append(
            attach_evidence(
                card,
                question_source_id=SOURCE_IDS["paper_answer"],
    ...
    translation["explanation"] = "参考译文已按本地百度网盘 2018 英语二答案速查 PDF 匹配，并与逐题细解 PDF 英汉对照复核。"
    cards.append(
        attach_evidence(
            translation,
            question_source_id=SOURCE_IDS["paper_answer"],
    ...
    return {
        ...
        "sourceFiles": [
            {
                "id": "english2-2018-document-version",
            ...
            {
                "id": "english2-2018-original-paper",
            ...
            {
                "id": "english2-2018-detailed-analysis",
...
def validate_cards(cards: list[dict[str, Any]], material: dict[str, Any], text: str, detail_text: str) -> None:
    ...
    if (
        "negative Internet comments" not in material["cloze_passage"]
        or "Christopher Hsee" not in material["cloze_passage"]
    ):
        issues.append("cloze passage missing curiosity/Hsee anchors")
    for number in range(1, 21):
        if f"({number})" not in material["cloze_passage"]:
            issues.append(f"cloze passage missing blank marker ({number})")
    if "Stephen Koziatek" not in material["passages"]["text1"] or "Manchester School of Technology" not in material["passages"]["text1"]:
        issues.append("Text 1 passage missing Koziatek/Manchester anchors")
    if "renewable" not in material["passages"]["text2"] or "Iowa" not in material["passages"]["text2"]:
        issues.append("Text 2 passage missing renewable/Iowa anchors")
    if "WhatsApp" not in material["passages"]["text3"] or "Whole Foods" not in material["passages"]["text3"]:
        issues.append("Text 3 passage missing WhatsApp/Whole Foods anchors")
    if "Cal Newport" not in material["passages"]["text4"] or "Srini Pillay" not in material["passages"]["text4"]:
        issues.append("Text 4 passage missing Newport/Pillay anchors")
    if "Five ways to make conversation with anyone" not in material["passages"]["part-b"] or "Skip the small talk" not in json.dumps(PART_B_OPTIONS):
        issues.append("Part B passage/options missing conversation anchors")
    if "Bill Gates" not in material["translation_text"] or "50 nonfiction books" not in material["translation_text"]:
        issues.append("translation text missing Gates/nonfiction anchors")
    if "英语（二）试题" in material["translation_text"]:
        issues.append("translation text contains PDF page footer")
    if "2017年某市消费者选择餐厅时的关注因素" not in material["writing_prompts"][48] or "特色36.3%" not in material["writing_prompts"][48]:
        issues.append("writing prompt 48 missing restaurant chart anchor")
    if "DBCBA" not in text or "AFEBD" not in text:
