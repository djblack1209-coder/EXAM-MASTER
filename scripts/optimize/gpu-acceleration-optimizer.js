...
const performanceRules = {
    // P0: 严重性能问题（触发重排）
    critical: [
        { pattern: /animation:.*?(width|height|top|left|right|bottom|margin|padding)/g, message: '动画使用了触发重排的属性' },
        { pattern: /transition:.*?(width|height|top|left|right|bottom|margin|padding)/g, message: '过渡使用了触发重排的属性' }
    ],
    // P1: 重要优化（缺少GPU加速提示）
    important: [
        { pattern: /@keyframes\s+(\w+)\s*{[^}]*transform:[^}]*}/g, message: '动画缺少 will-change 提示', needsWillChange: true },
