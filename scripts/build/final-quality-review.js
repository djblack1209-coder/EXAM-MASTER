...
- **缺少will-change**: ${results.codeQuality.missingWillChange.length}个文件
  ${results.codeQuality.missingWillChange
    .slice(0, 5)
    .map((f) => `  - ${f}`)
    .join('\n')}
...
${
  results.codeQuality.hardcodedColors.length > 0
    ? `
#### 硬编码颜色详情
${results.codeQuality.hardcodedColors
  .slice(0, 10)
...
function calculateScore() {
  ...
  // 性能评分 (25分)
  const gpuCoverage = results.components.total > 0 ? results.performance.gpuAccelerated / results.components.total : 0;
  score.performance = Math.min(
    25,
    gpuCoverage * 15 +
