...
const CONFIG = {
  // 源目录
  sourceDir: path.join(process.cwd(), 'src/static'),
  // 支持的输入格式
...
async function main() {
  ...
  try {
    ...
    if (typeof sharpTransform !== 'function') {
  ...
  for (const inputPath of imageFiles) {
    ...
    try {
      ...
      await sharpTransform(inputPath).webp({ quality: CONFIG.quality }).toFile(outputPath);
