...
// SQL 注入危险字符（虽然 MongoDB 不用 SQL，但仍需防范）
 ...
 * 检查是否包含 MongoDB 注入风险
 */
export function hasInjectionRisk(input: any): boolean {
 ...
 * 校验用户登录参数
 */
export function validateLoginParams(data: any): ValidationResult {
 ...
 * 校验错题操作参数
 */
export function validateMistakeParams(data: any): ValidationResult {
  return validate(data, {
 ...
 * 校验排行榜参数
 */
export function validateRankParams(data: any): ValidationResult {
  return validate(data, {
    action: CommonRules.action,
    uid: {
      type: 'string',
 ...
 * 校验社交服务参数
 */
export function validateSocialParams(data: any): ValidationResult {
  return validate(data, {
    action: CommonRules.action,
    userId: {
      type: 'string',
