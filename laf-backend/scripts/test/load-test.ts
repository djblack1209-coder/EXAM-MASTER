...
// 模拟请求
async function makeRequest(endpoint: string, payload: Record<string, unknown>): Promise<{ success: boolean; duration: number; error?: string }> {
  ...
  try {
    ...
    return { success: false, duration, error: error instanceof Error ? error.message : 'Network error' }
  }
}
