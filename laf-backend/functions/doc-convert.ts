...
export default async function (ctx) {
  ...
  console.log(`[${requestId}] ctx.body:`, JSON.stringify(ctx.body || {}).substring(0, 200))
  console.log(`[${requestId}] ctx.query:`, JSON.stringify(ctx.query || {}).substring(0, 200))
  console.log(`[${requestId}] ctx.request?.body:`, JSON.stringify(ctx.request?.body || {}).substring(0, 200))
  console.log(`[${requestId}] ctx.params:`, JSON.stringify(ctx.params || {}).substring(0, 200))
  ...
  try {
    ...
    if (typeof body === 'string') {
      try {
        ...
        console.error(`[${requestId}] body 解析失败:`, e.message)
      }
    }
    ...
    console.log(`[${requestId}] 最终 action:`, action)
    console.log(`[${requestId}] 最终 params keys:`, Object.keys(params))

    // S003: 入口参数校验
    const entryValidation = validate({ action }, {
      action: { required: true, type: 'string', maxLength: 50, enum: ['convert', 'get_status', 'get_types', 'get_result'] }
    })
    ...
    console.error(`[${requestId}] 文档转换异常:`, error)
    return {
      code: 500,
      success: false,
...
async function handleConvert(params, requestId) {
  ...
  console.log(`[${requestId}] 开始转换: ${convertType}, 文件大小: ${(fileSize / 1024).toFixed(2)}KB`)
  ...
  try {
    ...
    console.error(`[${requestId}] 转换失败:`, error)
    return {
      code: 500,
      success: false,
...
async function createConvertJob(convertType, fileBase64, fileName, outputFormat, options, requestId) {
  ...
  console.log(`[${requestId}] 创建任务: ${inputFormat} -> ${targetFormat}`)
  ...
  try {
    ...
    if (response.status !== 200 && response.status !== 201) {
      console.error(`[${requestId}] CloudConvert 创建任务失败:`, response.data)
      return {
        code: 500,
        success: false,
    ...
    console.log(`[${requestId}] 任务创建成功, jobId: ${jobData?.id}`)
    ...
    console.error(`[${requestId}] 创建任务异常:`, error)
    throw error
  }
}
...
async function waitForJobComplete(jobId, requestId, maxWait = 120000) {
  ...
  console.log(`[${requestId}] 等待任务完成: ${jobId}`)
  ...
  while (Date.now() - startTime < maxWait) {
    try {
      ...
      console.log(`[${requestId}] 任务状态: ${status}`)
      ...
      console.error(`[${requestId}] 查询状态失败:`, error)
      // 继续重试
      await sleep(pollInterval)
    }
  }
...
async function getConvertResult(params, requestId) {
  ...
  try {
      ...
      const downloadedFiles = await Promise.all(
        files.map(async (f) => {
          try {
            ...
            console.error(`下载文件失败: ${f.filename}`, e)
            return {
              filename: f.filename,
              url: f.url,
              error: '下载失败'
    ...
    console.error(`[${requestId}] 获取结果失败:`, error)
    return {
      code: 500,
      success: false,
