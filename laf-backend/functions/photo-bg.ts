  ...
  try {
    ...
    const entryValidation = validate({ action }, {
      action: { required: true, type: 'string', maxLength: 50, enum: [
        'remove_bg', 'change_bg', 'process', 'beauty', 'get_sizes', 'get_colors'
      ]}
    })
    ...
    console.error(`[${requestId}] 证件照处理异常:`, error)
    return {
      code: 500,
      success: false,
...
async function removeBackground(params, requestId) {
  ...
  console.log(`[${requestId}] 开始人像抠图, 图片大小: ${(fileSize / 1024).toFixed(2)}KB`)

  try {
    // 调用腾讯云人像分割 API
    const result = await callTencentAPI('bda', 'SegmentPortraitPic', {
      Image: imageBase64,
      RspImgType: 'Foreground' // 返回前景图（人像）
    }, requestId)

    if (result.Response?.Error) {
      console.error(`[${requestId}] 腾讯云 API 错误:`, result.Response.Error)
      return {
        code: 500,
        success: false,
    ...
    console.error(`[${requestId}] 抠图失败:`, error)
    return {
      code: 500,
      success: false,
...
async function changeBackground(params, requestId) {
  ...
  console.log(`[${requestId}] 更换背景颜色: ${targetColorHex}`)
  ...
  try {
    ...
    console.error(`[${requestId}] 换背景失败:`, error)
    return {
      code: 500,
      success: false,
...
async function processPhoto(params, requestId) {
  ...
  console.log(`[${requestId}] 完整处理: 尺寸=${size}, 背景=${targetColorHex}, 美颜=${beauty}`)
  ...
  try {
    ...
    // 步骤1: 美颜处理（可选）
    if (beauty) {
      console.log(`[${requestId}] 步骤1: 美颜处理...`)
      try {
        const beautyResult = await callTencentAPI('fmu', 'BeautifyPic', {
        ...
        if (beautyResult.Response?.ResultImage) {
          ...
          console.log(`[${requestId}] 美颜完成`)
        } else if (beautyResult.Response?.Error) {
          console.warn(`[${requestId}] 美颜失败:`, beautyResult.Response.Error.Message)
        }
      } catch (e) {
        console.warn(`[${requestId}] 美颜处理异常，继续处理原图:`, e.message)
      }
    }

    // 步骤2: 人像分割
    console.log(`[${requestId}] 步骤2: 人像分割...`)
    const segResult = await callTencentAPI('bda', 'SegmentPortraitPic', {
      Image: processedImage,
...
async function applyBeauty(params, requestId) {
  ...
  console.log(`[${requestId}] 美颜处理: 美白=${whitening}, 磨皮=${smoothing}`)

  try {
    const result = await callTencentAPI('fmu', 'BeautifyPic', {
      Image: imageBase64,
      Whitening: whitening,
      Smoothing: smoothing,
    ...
    console.error(`[${requestId}] 美颜失败:`, error)
    return {
      code: 500,
      success: false,
...
async function callTencentAPI(service, action, params, requestId) {
  ...
  console.log(`[${requestId}] 调用腾讯云 API: ${service}/${action}`)
  ...
  try {
    ...
    console.error(`[${requestId}] 腾讯云 API 调用失败:`, error)
    throw error
  }
}
