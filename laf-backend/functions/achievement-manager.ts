...
import {
  ...
  validateUserId, checkRateLimit, logger,
  generateRequestId, wrapResponse
...
const ACHIEVEMENTS = [
  // 学习里程碑
  { id: 'first_login', name: '初来乍到', description: '首次登录应用', icon: '🎉', category: 'milestone', condition: { type: 'login_count', value: 1 } },
  { id: 'study_3_days', name: '三天打鱼', description: '累计学习3天', icon: '📅', category: 'milestone', condition: { type: 'total_study_days', value: 3 } },
  { id: 'study_7_days', name: '一周坚持', description: '累计学习7天', icon: '🗓️', category: 'milestone', condition: { type: 'total_study_days', value: 7 } },
  { id: 'study_30_days', name: '月度学霸', description: '累计学习30天', icon: '📆', category: 'milestone', condition: { type: 'total_study_days', value: 30 } },
  { id: 'study_100_days', name: '百日冲刺', description: '累计学习100天', icon: '💯', category: 'milestone', condition: { type: 'total_study_days', value: 100 } },

  // 刷题成就
  { id: 'first_question', name: '初试牛刀', description: '完成第1道题', icon: '✏️', category: 'practice', condition: { type: 'total_questions', value: 1 } },
  { id: 'questions_50', name: '小试身手', description: '累计完成50道题', icon: '📝', category: 'practice', condition: { type: 'total_questions', value: 50 } },
  { id: 'questions_200', name: '刷题达人', description: '累计完成200道题', icon: '🎯', category: 'practice', condition: { type: 'total_questions', value: 200 } },
  { id: 'questions_500', name: '题海战术', description: '累计完成500道题', icon: '🏊', category: 'practice', condition: { type: 'total_questions', value: 500 } },
  { id: 'questions_1000', name: '千题斩', description: '累计完成1000道题', icon: '⚔️', category: 'practice', condition: { type: 'total_questions', value: 1000 } },

  // 正确率成就
  { id: 'accuracy_60', name: '及格万岁', description: '正确率达到60%', icon: '✅', category: 'accuracy', condition: { type: 'accuracy', value: 60 } },
  { id: 'accuracy_80', name: '优秀学员', description: '正确率达到80%', icon: '🌟', category: 'accuracy', condition: { type: 'accuracy', value: 80 } },
  { id: 'accuracy_95', name: '学神降临', description: '正确率达到95%', icon: '👑', category: 'accuracy', condition: { type: 'accuracy', value: 95 } },

  // 连续学习成就
  { id: 'streak_3', name: '三连击', description: '连续学习3天', icon: '🔥', category: 'streak', condition: { type: 'streak_days', value: 3 } },
  { id: 'streak_7', name: '周周不断', description: '连续学习7天', icon: '🔥', category: 'streak', condition: { type: 'streak_days', value: 7 } },
  { id: 'streak_30', name: '铁杵磨针', description: '连续学习30天', icon: '💪', category: 'streak', condition: { type: 'streak_days', value: 30 } },

  // 错题相关
  { id: 'first_mistake_mastered', name: '知错能改', description: '首次掌握一道错题', icon: '🔄', category: 'mistake', condition: { type: 'mastered_mistakes', value: 1 } },
  { id: 'mistakes_mastered_10', name: '错题克星', description: '掌握10道错题', icon: '🎖️', category: 'mistake', condition: { type: 'mastered_mistakes', value: 10 } },
  { id: 'mistakes_mastered_50', name: '错题终结者', description: '掌握50道错题', icon: '🏆', category: 'mistake', condition: { type: 'mastered_mistakes', value: 50 } },

  // 社交成就
  { id: 'first_pk', name: '初次交锋', description: '完成第1次PK对战', icon: '⚡', category: 'social', condition: { type: 'pk_count', value: 1 } },
  { id: 'pk_win_5', name: '五连胜', description: 'PK对战赢5次', icon: '🏅', category: 'social', condition: { type: 'pk_wins', value: 5 } }
]
...
async function handleGetAll(userId: string, data: Record<string, unknown>, requestId: string) {
  ...
  const unlockedIds = new Set(
    (user.data?.achievements || []).map((a: Record<string, unknown>) => a.id)
  )
...
async function handleCheck(userId: string, data: Record<string, unknown>, requestId: string) {
  ...
  try {
    const mistakeResult = await db.collection('mistake_book').where({
      user_id: userId,
      is_mastered: true
    }).count()
  ...
  // 计算正确率
  const accuracy = userData.total_questions > 0
    ? Math.round((userData.correct_questions / userData.total_questions) * 100)
    : 0
