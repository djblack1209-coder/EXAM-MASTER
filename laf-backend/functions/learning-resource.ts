...
import { checkRateLimit } from './_shared/api-response'
...
// ==================== 日志工具 ====================
const logger = {
  info: (...args: unknown[]) => console.log('[LearningResource]', ...args),
  warn: (...args: unknown[]) => console.warn('[LearningResource]', ...args),
  error: (...args: unknown[]) => console.error('[LearningResource]', ...args)
}
  ...
  try {
    ...
    const entryValidation = validate({ action }, {
      action: { required: true, type: 'string', maxLength: 50, enum: [
        'getRecommendations', 'getHotResources', 'getByCategory', 'search',
        'favorite', 'getUserFavorites', 'recordProgress', 'getStats',
...
async function handleGetRecommendations(userId: string, data: Record<string, unknown>, requestId: string) {
  ...
  const result = await db.collection('learning_resources')
    .where(query)
...
async function handleGetHotResources(userId: string, data: Record<string, unknown>, requestId: string) {
  ...
  // 添加排名
  resources.forEach((r, i) => { r.rank = i + 1 })
...
async function handleSearch(userId: string, data: Record<string, unknown>, requestId: string) {
  ...
  logger.info(`[${requestId}] 搜索 "${trimmedKeyword}": ${total} 条结果`)
...
async function handleGetUserFavorites(userId: string, data: Record<string, unknown>, requestId: string) {
  ...
  if (resourceIds.length > 0) {
    const resourceRes = await db.collection('learning_resources')
...
async function handleGetStats(userId: string, data: Record<string, unknown>, requestId: string) {
  ...
  const [progressRes, favoriteRes] = await Promise.all([
    progressCollection.where({ user_id: userId }).field({ resource_id: true, progress: true, total_duration: true, last_study_at: true }).limit(500).get(),
  ...
  return {
    ...
    data: {
      ...
      recentProgress: progressList
        .sort((a, b) => (b.last_study_at || 0) - (a.last_study_at || 0))
        .slice(0, 5)
    },
    message: '获取成功'
  }
