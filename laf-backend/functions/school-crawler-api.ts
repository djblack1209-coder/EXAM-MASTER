...
// 省份代码反向映射
const PROVINCE_CODE_MAP = Object.entries(PROVINCE_MAP).reduce((acc, [code, name]) => {
  acc[name] = code
  return acc
...
export default async function (ctx: FunctionContext) {
  ...
  try {
    ...
    const entryValidation = validate({ action }, {
      action: { required: true, type: 'string', maxLength: 50, enum: ['list', 'refresh', 'by_province', 'provinces', 'status', 'crawl_all'] }
    })
    if (!entryValidation.valid) {
      return { code: 400, message: entryValidation.errors[0], requestId }
    }
    
    console.log(`[${requestId}] 爬虫API: action=${action}`)
    ...
    console.error(`[${requestId}] 爬虫API异常:`, error)
    return {
      code: 500,
...
async function getSchoolList(data: Record<string, unknown>, requestId: string) {
  ...
  try {
    ...
    console.error(`[${requestId}] 爬取失败:`, error)
    return await getSchoolsFromDB({ page, pageSize, province }, requestId)
  }
}
...
async function checkCacheValid() {
  try {
    const meta = await db.collection('crawler_meta')
      .where({ type: 'schools' })
      .getOne()
...
function is985School(code: string): boolean {
  const codes985 = [
    '10001', '10002', '10003', '10006', '10007', '10008', '10010', '10019', '10027', '10055',
    '10141', '10145', '10183', '10213', '10246', '10247', '10248', '10251', '10284', '10286',
    '10335', '10358', '10384', '10422', '10486', '10487', '10491', '10532', '10533', '10558',
    '10610', '10611', '10614', '10698', '10699', '10712', '10730', '91002', '91030'
  ]
...
function is211School(code: string): boolean {
  ...
  const codes211 = [
    // 985院校（39所，都是211）
    '10001', '10002', '10003', '10006', '10007', '10019', '10027', '10034',
    '10055', '10056', '10141', '10145', '10183', '10213', '10246', '10247',
    '10248', '10269', '10284', '10286', '10335', '10358', '10384', '10422',
    '10423', '10486', '10487', '10532', '10533', '91002', '10558', '10561',
    '10610', '10614', '10611', '10698', '10699', '10712', '10730',
    // 非985的211院校（73所）
    '10004', '10005', '10008', '10010', '10011', '10013', '10026', '10030',
    '10033', '10036', '10043', '10053', '10054', '10051', '10052', '10058',
    '10140', '10151', '10184', '10200', '10217', '10225', '10226', '10251',
    '10255', '10264', '10268', '10271', '10272', '10280', '10285', '10287',
    '10288', '10290', '10294', '10295', '10307', '10319', '10357', '10359',
    '10386', '10403', '10425', '10459', '10491', '10497', '10504', '10511',
    '10520', '10530', '10542', '10559', '10574', '10593', '10589', '10613',
    '10626', '10651', '10657', '10673', '10697', '10701', '10710', '10718',
    '10736', '10743', '10749', '10755', '10759', '10126', '10694', '10080'
  ]
...
async function refreshSchoolData(data: Record<string, unknown>, requestId: string) {
  ...
  console.log(`[${requestId}] 开始强制刷新院校数据`)
  ...
  try {
    ...
    for (const school of schools) {
      try {
        const existing = await db.collection(CACHE_COLLECTION)
        ...
        console.error(`保存院校 ${school.code} 失败:`, saveError)
      }
    }
    ...
    console.error(`[${requestId}] 刷新失败:`, error)
    return {
      code: 500,
...
async function getProvinceStats(requestId: string) {
  ...
  if (cacheValid) {
    const result = await db.collection(CACHE_COLLECTION)
      .aggregate()
      .group({
        _id: '$province',
        count: { $sum: 1 },
    ...
    const provinces = (result.data as Record<string, unknown>[])?.map(p => ({
      name: p._id,
...
async function crawlAllSchools(requestId: string) {
  console.log(`[${requestId}] 开始全量爬取院校数据`)
  ...
  try {
    ...
    for (const provinceCode of provinceCodes) {
      try {
        console.log(`[${requestId}] 爬取省份: ${PROVINCE_MAP[provinceCode]} (${provinceCode})`)
          ...
          try {
            ...
            for (const school of schools) {
              try {
                ...
                console.error(`保存院校 ${school.code} 失败:`, saveError)
                errorCount++
                errors.push({ code: school.code, error: saveError instanceof Error ? saveError.message : String(saveError) })
              }
            }
            ...
            console.error(`[${requestId}] 爬取省份 ${provinceCode} 第 ${start} 条失败:`, pageError)
            errorCount++
            errors.push({ province: provinceCode, start, error: pageError instanceof Error ? pageError.message : String(pageError) })
            hasMore = false
          }
        }
        ...
        console.error(`[${requestId}] 爬取省份 ${provinceCode} 失败:`, provinceError)
        errorCount++
        errors.push({ province: provinceCode, error: provinceError instanceof Error ? provinceError.message : String(provinceError) })
      }
    }
    ...
    const existingMeta = await db.collection('crawler_meta')
      .where({ type: 'schools' })
      .getOne()
    
    if (existingMeta.data) {
      await db.collection('crawler_meta')
        .doc(existingMeta.data._id)
        .update(metaData)
    } else {
    ...
    console.log(`[${requestId}] 全量爬取完成: 成功 ${crawledCount}, 失败 ${errorCount}`)
    ...
    console.error(`[${requestId}] 全量爬取异常:`, error)
    return {
      code: 500,
