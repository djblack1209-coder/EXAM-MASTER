...
import { filterCategoryStats, getCategoryAllowlistQuery } from './_shared/env-guard.js';
...
async function getCategoryStats(requestId) {
    try {
        ...
        const stats = filterCategoryStats(Object.values(categories).map((c) => ({
            ...c,
            sub_categories: Object.entries(c.sub_categories).map(([name, count]) => ({ name, count }))
        })));
        const total = stats.reduce((sum, c) => sum + c.total, 0);
        return {
            code: 0,
            success: true,
