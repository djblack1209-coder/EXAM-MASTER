...
async function handleGetOverview(userId: string, data: Record<string, unknown>, requestId: string) {
  ...
  return success({
    ...
    mistakeMasteryRate: mistakeCountRes.total > 0
      ? Math.round(((masteredCountRes.total || 0) / mistakeCountRes.total) * 100)
      : 0,
    ...
    today: {
      ...
      accuracy: todayStatsRes?.questions > 0
        ? Math.round(((todayStatsRes?.correct || 0) / todayStatsRes.questions) * 100)
        : 0
    },
...
async function handleGetDailyStats(userId: string, data: Record<string, unknown>, requestId: string) {
  ...
  const records = await db.collection('daily_stats').where({
    user_id: userId,
    date: _.gte(startDate.getTime()).and(_.lte(endDate.getTime()))
  }).orderBy('date', 'asc').limit(limitDays).get()
  ...
  for (let i = 0; i < limitDays; i++) {
    ...
    dailyStats.push({
      ...
      accuracy: record?.questions > 0
        ? Math.round(((record?.correct || 0) / record.questions) * 100)
        : 0
    })
  }
...
async function handleGetTrend(userId: string, data: Record<string, unknown>, requestId: string) {
  ...
  const avgFirst = firstHalf.length > 0
    ? firstHalf.reduce((s, d) => s + d.questions, 0) / firstHalf.length
    : 0
  const avgSecond = secondHalf.length > 0
    ? secondHalf.reduce((s, d) => s + d.questions, 0) / secondHalf.length
    : 0

  const trend = avgSecond > avgFirst ? 'up' : avgSecond < avgFirst ? 'down' : 'stable'
  const trendPercent = avgFirst > 0
...
async function handleGetRankInfo(userId: string, data: Record<string, unknown>, requestId: string) {
  ...
  const higherCount = await db.collection('users').where({
  ...
  const percentile = totalUsers.total > 0
    ? Math.round(((totalUsers.total - rank) / totalUsers.total) * 100)
    : 0
  ...
  return success({
    ...
    description: percentile >= 90 ? '超越了90%的用户' :
                 percentile >= 70 ? '超越了70%的用户' :
                 percentile >= 50 ? '超越了50%的用户' :
                 '继续加油！'
  }, '获取成功')
}
...
async function getDailyRecord(userId: string, dateKey: string) {
  const result = await db.collection('daily_stats').where({
    user_id: userId,
    date_key: dateKey
  }).getOne()
