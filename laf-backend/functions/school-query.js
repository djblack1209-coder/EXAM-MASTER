...
export default async function (ctx) {
  ...
  try {
    ...
    console.log(`[${requestId}] action=${action} 耗时 ${duration}ms`)
    return result
...
async function checkDataHealth(requestId) {
  ...
  const cached = getCache(cacheKey)
  if (cached) {
    console.log(`[${requestId}] 学校详情缓存命中: ${schoolId || code}`)
...
async function searchSchools(data, requestId) {
  ...
  const cached = getCache(cacheKey)
  if (cached) {
    console.log(`[${requestId}] 搜索缓存命中: "${searchTerm}"`)
    return { ...cached, requestId }
  }
...
async function getHotSchools(data, requestId) {
  ...
  const cached = getCache(cacheKey)
  if (cached) {
    console.log(`[${requestId}] 热门学校缓存命中`)
    return { ...cached, requestId }
  }
...
async function getColleges(data, requestId) {
  ...
  // 查询该校所有专业，按学院分组统计
  const allMajors = await db.collection('majors')
    .where({ schoolId })
  ...
  const list = Array.from(collegeMap.values()).map(c => ({
    ...c,
...
async function getMajorDetail(data, requestId) {
  ...
  const [scoreLines, ratios] = await Promise.all([
    db.collection('score_lines')
      ...
      .limit(5)
      .get(),
    db.collection('admission_ratios')
      .where({ majorId })
      .orderBy('year', 'desc')
      .limit(5)
      .get()
...
async function getStats(requestId) {
  ...
  const cached = getCache(cacheKey)
  if (cached) {
    console.log(`[${requestId}] 统计数据缓存命中`)
    return { ...cached, requestId }
  }
...
async function getProvinces(requestId) {
  ...
  const cached = getCache(cacheKey)
  if (cached) {
    console.log(`[${requestId}] 省份列表缓存命中`)
    return { ...cached, requestId }
  }
...
async function syncFromChsi(data, requestId) {
  ...
  console.log(`[${requestId}] 开始同步 ${schools.length} 个院校数据`)
  ...
  console.log(`[${requestId}] 已清理所有学校相关缓存`)
  
  // 获取同步后的状态（缓存已清理，会重新查询）
  const dataStatus = await checkDataCompleteness()
  
  console.log(`[${requestId}] 同步完成: 新增=${inserted}, 更新=${updated}, 失败=${failed}`)
