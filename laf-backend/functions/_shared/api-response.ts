 ...
 * 错误响应
 */
export function error(code: number = ResponseCode.INTERNAL_ERROR, message?: string, errorDetail?: string): ApiResponse {
  return {
    code,
    success: false,
 ...
 * 包装响应，添加请求ID和耗时
 */
export function wrapResponse<T>(response: ApiResponse<T>, requestId: string, startTime: number): ApiResponse<T> {
  return {
    ...response,
    requestId,
...
export function validateAction(action: unknown): boolean {
  return typeof action === 'string' && action.length > 0 && action.length <= 50 && /^[a-zA-Z_]+$/.test(action);
}
...
const RATE_LIMIT_COLLECTION = 'rate_limit_records';
const RATE_LIMIT_EXPIRE_GRACE = 5 * 60 * 1000;

function buildRateLimitDocId(key: string, windowMs: number, windowStart: number): string {
  return `rl_${crypto.createHash('sha1').update(`${key}:${windowMs}:${windowStart}`).digest('hex')}`;
}
...
/**
 * 分布式速率限制（Serverless 友好）
 * 使用数据库窗口计数，跨实例保持一致；失败时自动降级到内存限流
 */
export async function checkRateLimitDistributed(
  key: string,
  limit: number,
  windowMs: number,
  fallback?: () => { allowed: boolean; remaining: number; resetAt: number }
): Promise<{ allowed: boolean; remaining: number; resetAt: number; source: 'db' | 'memory' }> {
  ...
  const docId = buildRateLimitDocId(key, safeWindow, windowStart);

  try {
    const collection = db.collection(RATE_LIMIT_COLLECTION);
    const docRef = collection.doc(docId);
    const existingRes = await docRef.get();
    ...
    if (!existing || existing.resetAt <= now) {
      if (existing) {
        await docRef.update({
          ...
          resetAt,
          windowMs: safeWindow,
          expireAt: resetAt + RATE_LIMIT_EXPIRE_GRACE,
          updatedAt: now
        });
      } else {
        try {
          ...
          const retryRes = await docRef.get();
          const retry = retryRes?.data;

          if (retry && retry.resetAt > now) {
            const retryCount = Number(retry.count || 0);
            if (retryCount >= safeLimit) {
              return { allowed: false, remaining: 0, resetAt: Number(retry.resetAt) || resetAt, source: 'db' };
            }
            ...
            return {
              ...
              resetAt: Number(retry.resetAt) || resetAt,
              source: 'db'
            };
          }

          throw new Error('rate_limit_create_race');
        }
      }

      return { allowed: true, remaining: Math.max(0, safeLimit - 1), resetAt, source: 'db' };
    }

    const currentCount = Number(existing.count || 0);
    const currentResetAt = Number(existing.resetAt || resetAt);

    if (currentCount >= safeLimit) {
      return { allowed: false, remaining: 0, resetAt: currentResetAt, source: 'db' };
    }
    ...
    const localResult = typeof fallback === 'function' ? fallback() : checkRateLimit(key, safeLimit, safeWindow);
    return {
      ...localResult,
      source: 'memory'
    };
