 ...
 */
export function generateIdempotencyKey(userId: string, action: string, clientKey: string): string {
...
export async function checkIdempotency(options: IdempotencyOptions): Promise<IdempotencyResult> {
  ...
  try {
    // 安全修复：使用 upsert 原子操作替代 check-then-insert，消除 TOCTOU 竞态
    // 先尝试查找现有记录
    ...
    // 5. 创建新记录 — 使用唯一键约束防止并发插入
    // 如果 key 字段有唯一索引，并发插入会抛出 duplicate key error
    ...
    try {
      ...
      return {
        isNew: true,
        isDuplicate: false,
        recordId: String(insertResult.id)
      };
    } catch (insertError: any) {
      // 并发插入冲突（唯一索引冲突），视为重复请求
      if (insertError?.code === 11000 || insertError?.message?.includes('duplicate')) {
        logger.warn(`[Idempotency] 并发插入冲突，视为重复请求: ${fullKey}`);
        return {
          isNew: false,
          isDuplicate: true,
          previousResult: { code: 429, message: '请求正在处理中，请稍后' }
 ...
 * 标记请求完成
 */
export async function markCompleted(recordId: string, result: unknown): Promise<void> {
  if (!recordId) return;
 ...
 * 标记请求失败
 */
export async function markFailed(recordId: string, error: unknown): Promise<void> {
  if (!recordId) return;
 ...
 * )
 */
export async function withIdempotency<T>(options: IdempotencyOptions, handler: () => Promise<T>): Promise<T> {
  // 1. 检查幂等性
