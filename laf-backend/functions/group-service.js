...
// ==================== 日志工具 ====================
const logger = {
  info: (...args) => { if (LOG_LEVEL !== 'warn' && LOG_LEVEL !== 'error') console.log(...args) },
  warn: (...args) => { if (LOG_LEVEL !== 'error') console.warn(...args) },
...
async function handleGetGroups(userId, params, requestId) {
  ...
  try {
    ...
    if (type === 'joined') {
      // 获取用户加入的小组
      const memberGroups = await db.collection('group_members')
...
async function handleLeaveGroup(userId, params, requestId) {
  ...
  try {
    ...
    // 更新成员状态
    await db.collection('group_members')
...
async function handleGetResources(userId, params, requestId) {
  ...
  try {
    ...
    let query = db.collection('group_resources')
