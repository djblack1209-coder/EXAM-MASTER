 ...
 * @returns 验证结果
 */
function verifyUserAccess(ctx: Record<string, unknown>, targetUserId: string): { valid: boolean; error?: string; tokenUserId?: string } {
  // 从请求头获取token
...
function sanitizePracticeConfig(config: unknown): PracticeConfig {
  ...
  return {
    ...
    show_answer_immediately: typeof config.show_answer_immediately === 'boolean' ? config.show_answer_immediately : defaults.show_answer_immediately,
    ...
    difficulty_preference: validDifficulties.includes(config.difficulty_preference) ? config.difficulty_preference : defaults.difficulty_preference,
    categories: Array.isArray(config.categories) ? config.categories.slice(0, 10).map(c => sanitizeString(c, 20)) : defaults.categories,
    daily_goal: Math.min(Math.max(parseInt(config.daily_goal) || defaults.daily_goal, 10), 500),
    reminder_enabled: typeof config.reminder_enabled === 'boolean' ? config.reminder_enabled : defaults.reminder_enabled,
