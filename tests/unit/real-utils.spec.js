...
// ============================================================
// 4. config-validator.js 测试
// ============================================================

// 需要 mock logger 和 import.meta.env
...
describe('config-validator.js', () => {
  // 保存原始 import.meta.env
  ...
  // 动态导入以便每次测试可以控制 env
  // 由于 config-validator 在模块顶层读取 env，我们直接测试导出函数
  // 函数内部通过 getEnvValue 动态读取 import.meta.env

  describe('validateConfig', () => {
    it('所有配置存在时返回 valid=true', async () => {
      import.meta.env.VITE_WX_APP_ID = 'wx123456';
      import.meta.env.VITE_API_BASE_URL = 'https://api.example.com';
      import.meta.env.VITE_INVITE_SECRET = 'invite_secret_for_test';
    ...
    it('warning 级别不影响 valid 状态', async () => {
      ...
      // 所有 REQUIRED_CONFIGS 都是 warning 级别
      expect(result.valid).toBe(true);
    });
  });

  describe('printConfigStatus', () => {
    it('返回 valid 和 issues', async () => {
      const { printConfigStatus } = await import('@/utils/core/config-validator.js');
      const result = printConfigStatus();
  ...
  describe('getConfigStatusForAnalytics', () => {
    it('返回分析用的配置状态对象', async () => {
      const { getConfigStatusForAnalytics } = await import('@/utils/core/config-validator.js');
      const result = getConfigStatusForAnalytics();

      expect(result).toHaveProperty('config_valid');
      expect(result).toHaveProperty('config_missing_count');
      expect(result).toHaveProperty('config_missing_keys');
      expect(result).toHaveProperty('config_env');
      expect(result).toHaveProperty('config_timestamp');
      expect(typeof result.config_timestamp).toBe('number');
    });

    it('config_missing_keys 是逗号分隔的字符串', async () => {
      const { getConfigStatusForAnalytics } = await import('@/utils/core/config-validator.js');
      const result = getConfigStatusForAnalytics();

      expect(typeof result.config_missing_keys).toBe('string');
    });
  });
});
