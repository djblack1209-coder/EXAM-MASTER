...
describe('question bank public-course year map', () => {
  ...
  it('keeps every track on its real release-scope year map and marks verified banks as clickable', () => {
    ...
    expect(english2.yearSlots.find((slot) => slot.year === '2010')).toMatchObject({
      bankId: 'english2-2010',
      status: 'ready',
      statusLabel: '正式',
      actionLabel: '开始',
      clickable: true
    });
    expect(english2.yearSlots.find((slot) => slot.year === '2011')).toMatchObject({
      bankId: 'english2-2011',
      status: 'ready',
      statusLabel: '正式',
      actionLabel: '开始',
      clickable: true
    });
    expect(english2.yearSlots.find((slot) => slot.year === '2012')).toMatchObject({
      bankId: 'english2-2012',
      status: 'ready',
      statusLabel: '正式',
      actionLabel: '开始',
      clickable: true
    });
    expect(english2.yearSlots.find((slot) => slot.year === '2013')).toMatchObject({
      bankId: 'english2-2013',
      status: 'ready',
      statusLabel: '正式',
      actionLabel: '开始',
      clickable: true
    });
    expect(english2.yearSlots.find((slot) => slot.year === '2014')).toMatchObject({
      bankId: 'english2-2014',
      status: 'ready',
      statusLabel: '正式',
      actionLabel: '开始',
      clickable: true
    });
    expect(english2.yearSlots.find((slot) => slot.year === '2015')).toMatchObject({
      bankId: 'english2-2015',
      status: 'ready',
      statusLabel: '正式',
      actionLabel: '开始',
      clickable: true
    });
    expect(english2.yearSlots.find((slot) => slot.year === '2016')).toMatchObject({
      bankId: 'english2-2016',
      status: 'ready',
      statusLabel: '正式',
      actionLabel: '开始',
      clickable: true
    ...
    expect(english2.yearSlots.find((slot) => slot.year === '2019')).toMatchObject({
      bankId: 'english2-2019',
      status: 'ready',
      statusLabel: '正式',
      actionLabel: '开始',
      clickable: true
    });
    expect(english2.yearSlots.find((slot) => slot.year === '2020')).toMatchObject({
      bankId: 'english2-2020',
      status: 'ready',
      statusLabel: '正式',
      actionLabel: '开始',
      clickable: true
    ...
    expect(math3.slotSummary).toEqual({ total: 22, ready: 16, organizing: 0, missing: 6 });
    ...
    expect(math3.yearSlots.find((slot) => slot.year === '2006')).toMatchObject({
      bankId: 'math3-2006',
      status: 'ready',
      statusLabel: '正式',
      actionLabel: '开始',
    ...
    expect(math3.yearSlots.find((slot) => slot.year === '2007')).toMatchObject({
      bankId: 'math3-2007',
      status: 'ready',
      statusLabel: '正式',
      actionLabel: '开始',
    ...
    for (const year of ['2008', '2009', '2010', '2011', '2012', '2013', '2015', '2016', '2017', '2018', '2019']) {
