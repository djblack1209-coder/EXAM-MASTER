        ...
        <view class="achievement-section">
          ...
          <view class="achievement-grid">
            <view v-for="ach in unlockedAchievements" :key="ach.id" class="achievement-item unlocked">
              <text class="achievement-item-icon">
                {{ ach.icon }}
              </text>
...
.achievement-modal {
  width: 100%; max-width: 420px; max-height: 80vh; background: var(--bg-card);
  border-radius: 24px; padding: 32px; box-shadow: var(--shadow-xl);
  animation: fadeInUp 0.3s ease-out; overflow-y: auto;
}
.achievement-modal-header {
  display: flex; align-items: center; justify-content: space-between; margin-bottom: 24px;
}
.achievement-modal-title { font-size: 44rpx; font-weight: 700; color: var(--text-primary); }
.achievement-modal-close {
  width: 36px; height: 36px; display: flex; align-items: center; justify-content: center;
  font-size: 48rpx; color: var(--text-sub); cursor: pointer; border-radius: 50%; background: var(--bg-secondary);
}
.achievement-modal-body { padding-bottom: 16px; }
...
.achievement-item {
  ...
  border-radius: 16px; background: var(--bg-secondary); text-align: center;
}
.achievement-item.unlocked { background: linear-gradient(135deg, rgba(46, 204, 113, 0.1), rgba(52, 152, 219, 0.1)); }
.achievement-item.locked { opacity: 0.5; }
.achievement-item-icon { font-size: 40rpx; margin-bottom: 8px; }
.achievement-item-name { font-size: 24rpx; font-weight: 600; color: var(--text-primary); margin-bottom: 4px; }
.achievement-item-desc { font-size: 20rpx; color: var(--text-sub); line-height: 1.3; }
@keyframes fadeInUp {
