...
.practice-modes-modal-title { font-size: 44rpx; font-weight: 700; color: var(--text-primary); }
...
.practice-modes-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px; }
.practice-mode-item {
  display: flex; flex-direction: column; align-items: center; padding: 24px 16px;
  border-radius: 20px; background: var(--bg-secondary); text-align: center;
  border: 1px solid var(--border); transition: all 0.2s ease; cursor: pointer;
}
.practice-mode-item:active { transform: scale(0.97); background: var(--primary-light); }
.practice-mode-icon { font-size: 48rpx; margin-bottom: 12px; }
.practice-mode-name { font-size: 28rpx; font-weight: 600; color: var(--text-primary); margin-bottom: 8px; }
.practice-mode-desc { font-size: 22rpx; color: var(--text-sub); line-height: 1.4; }
@keyframes fadeInUp {
