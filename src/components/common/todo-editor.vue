 ...
 * 
 * 功能：
 * 1. 底部 ActionSheet 弹出编辑
 * 2. 支持新增/编辑/删除
 * 3. 优先级选择
 * 4. 拖拽排序（预留）
 */
<template>
  <view class="todo-editor-mask" v-if="visible" @tap="handleClose">
    <view 
    ...
    >
      ...
      <view class="editor-header">
        <text class="editor-title">{{ isEdit ? '编辑待办' : '新建待办' }}</text>
      ...
      <!-- 快捷操作 -->
      <view class="quick-actions" v-if="isEdit">
        <view class="action-item" @tap="handleToggleComplete">
          <text class="action-icon">{{ todoData.completed ? '↩️' : '✅' }}</text>
          <text class="action-text">{{ todoData.completed ? '标记未完成' : '标记完成' }}</text>
        </view>
