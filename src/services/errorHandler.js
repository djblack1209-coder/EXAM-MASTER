...
class ErrorHandler {
  ...
  /**
	 ...
	 * @param {Error} error - 错误对象
	 * @param {Object} context - 错误上下文
	 * @param {string} [context.page='unknown'] - 发生错误的页面
	 * @param {string} [context.action='unknown'] - 触发错误的操作
	 * @param {string} [context.userId='anonymous'] - 用户ID
  ...
  logError(errorInfo) {
    ...
    try {
      ...
      storageService.save('error_logs', storedLogs.slice(0, 50)); // 本地只保存50条
    } catch (e) {
  ...
  reportError(errorInfo) {
    ...
    logger.log('[ErrorHandler] 错误已记录，待上报:', errorInfo.message);
  }
  ...
  clearErrorLogs() {
    this.errorLog = [];
    try {
      storageService.remove('error_logs');
    } catch (e) {
  ...
  /**
	 * 处理Promise拒绝
	 * @param {Error|*} reason - Promise 拒绝原因
	 * @param {Promise} promise - 被拒绝的 Promise 对象
	 */
 ...
 * @param {Object} [context={}] - 错误上下文 (page, action, userId)
