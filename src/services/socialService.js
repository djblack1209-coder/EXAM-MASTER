...
export const socialService = {
  ...
  async searchUser(keyword) {
    try {
      console.log('[Social] 搜索用户:', keyword)
  ...
  async sendRequest(targetUid, message = '') {
    try {
      console.log('[Social] 发送好友请求:', { targetUid, message })
  ...
  async handleRequest(fromUid, action) {
    try {
      console.log('[Social] 处理好友请求:', { fromUid, action })
  ...
  async getFriendList(sortBy = 'score', useCache = true) {
    console.log('[Social] 获取好友列表:', { sortBy, useCache })
  ...
  async getFriendRequests() {
    try {
      console.log('[Social] 获取好友请求列表')
      
      const res = await lafService.socialService({
        action: 'get_friend_requests'
      })
      
      console.log('[Social] 好友请求列表:', res)
      return res
    } catch (err) {
  ...
  async removeFriend(friendUid) {
    try {
      console.log('[Social] 删除好友:', friendUid)
      ...
      if (res.code === 0) {
        ...
        console.log('[Social] 已清除缓存')
      }
      
      console.log('[Social] 删除结果:', res)
      return res
    } catch (err) {
