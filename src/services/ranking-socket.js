...
class RankingSocketService {
  ...
  _getWebSocketUrl() {
    ...
    const host = isDev ? 'localhost:3000' : 'api.exam-master.com'
    return `${protocol}://${host}/ws/ranking`
  }
  ...
  connect(options = {}) {
    ...
    if (this.socket && this.state.value === WS_STATE.OPEN) {
      console.log('[RankingSocket] Already connected')
   ...
   */
  _onOpen() {
    console.log('[RankingSocket] Connected')
  ...
  _onMessage(data) {
    try {
      ...
      switch (message.type) {
        ...
        default:
          console.log('[RankingSocket] Unknown message type:', message.type)
      }
  ...
  _handleRankingUpdate(data) {
    ...
    console.log('[RankingSocket] Ranking updated:', {
      count: list?.length,
      type
    })
  }
  ...
  _handlePositionChange(data) {
    ...
    console.log('[RankingSocket] Position changed:', {
      from: previousRank,
      to: rank,
   ...
   */
  _onClose(event) {
    console.log('[RankingSocket] Disconnected:', event?.code, event?.reason)
  ...
  _scheduleReconnect() {
    ...
    console.log(`[RankingSocket] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts})`)
