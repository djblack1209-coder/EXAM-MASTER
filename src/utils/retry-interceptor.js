...
class RetryInterceptor {
  ...
  setConfig(config) {
    ...
    console.log('[RetryInterceptor] 配置已更新:', this.config)
  }
  ...
  async executeWithRetry(requestFn, options = {}) {
    ...
    try {
      while (retryCount <= config.maxRetries) {
        ...
        try {
          ...
          if (retryCount > 0) {
            ...
            console.log(`[RetryInterceptor] 🔄 第 ${retryCount}/${config.maxRetries} 次重试...`)
          }
          ...
          if (retryCount > 0) {
            console.log(`[RetryInterceptor] ✅ 重试成功（第 ${retryCount} 次）`)
          }
          ...
          console.log(`[RetryInterceptor] ⏳ ${delayMs}ms 后进行第 ${retryCount + 1} 次重试...`)
      ...
      console.log(`[RetryInterceptor] ❌ 重试 ${retryCount} 次后仍然失败`)

      // 如果是网络错误，加入离线队列
      if (this.isNetworkError(lastError) && options.offlineQueueable !== false) {
        console.log('[RetryInterceptor] 📥 请求已加入离线队列')
  ...
  cancelRetry(requestId) {
    ...
    if (cancelFn) {
      ...
      this.retryingRequests.delete(requestId)
      this.cancelTokens.delete(requestId)
      console.log(`[RetryInterceptor] 🚫 已取消重试: ${requestId}`)
    }
  }
   ...
   */
  cancelAllRetries() {
    this.cancelTokens.forEach((cancelFn, requestId) => {
      cancelFn()
    })
    this.retryingRequests.clear()
    this.cancelTokens.clear()
    console.log('[RetryInterceptor] 🚫 已取消所有重试')
  }
    ...
    return {
      ...
      fail(err, args) {
        ...
        if (self.shouldRetry(err, retryConfig.retryCount)) {
          ...
          console.log(`[RetryInterceptor] 🔄 第 ${retryConfig.retryCount}/${self.config.maxRetries} 次重试，延迟 ${delay}ms`)
