 ...
 * @param {Object} [context={}] - 错误上下文 (page, action)
...
export function wrapAsync(fn, context = {}) {
  return async function (...args) {
    try {
      ...
      _handleError(context.type || 'Async Error', error);
      return null;
    }
  };
}
  ...
  try {
    ...
    if (result instanceof Promise) {
      return result.catch((error) => {
        _handleError(context.type || 'Safe Execute Error', error);
        return fallback;
      });
    }
    return result;
