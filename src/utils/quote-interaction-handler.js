...
class QuoteInteractionHandler {
  ...
  init() {
    try {
      this.favoriteQuotes = uni.getStorageSync(STORAGE_KEYS.FAVORITE_QUOTES) || [];
      this.shareCount = uni.getStorageSync(STORAGE_KEYS.SHARE_COUNT) || 0;
    } catch (e) {
  ...
  async toggleFavorite(quote, author = '古人云') {
    ...
    // 保存到本地
    uni.setStorageSync(STORAGE_KEYS.FAVORITE_QUOTES, this.favoriteQuotes);
  ...
  async shareQuote(quote, author, options = {}) {
    ...
    // 记录分享次数
    this.shareCount++;
    uni.setStorageSync(STORAGE_KEYS.SHARE_COUNT, this.shareCount);
    uni.setStorageSync(STORAGE_KEYS.LAST_SHARE_TIME, Date.now());
  ...
  getShareStats() {
    return {
      ...
      lastShareTime: uni.getStorageSync(STORAGE_KEYS.LAST_SHARE_TIME)
    };
  }
}
