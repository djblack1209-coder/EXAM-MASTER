...
export const eloMatchmaker = {
  ...
  getUserRating(userId = null) {
    try {
      const eloData = uni.getStorageSync(ELO_STORAGE_KEY) || {};
  ...
  updateUserRating(newRating, userId = null) {
    try {
      ...
      uni.setStorageSync(ELO_STORAGE_KEY, eloData);
      console.log('[ELOMatchmaker] 评分已更新:', eloData[key]);
    } catch (error) {
  ...
  async startMatching(options = {}) {
    ...
    console.log('[ELOMatchmaker] 开始匹配，用户评分:', userRating);
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeoutTimer = setTimeout(() => {
        ...
        console.log('[ELOMatchmaker] 匹配超时，分配机器人对手:', bot.name);
      ...
      setTimeout(() => {
        ...
        console.log('[ELOMatchmaker] 匹配成功:', {
  ...
  processMatchResult(result) {
    ...
    console.log('[ELOMatchmaker] 对战结果处理完成:', {
  ...
  recordMatchHistory(record) {
    try {
      const history = uni.getStorageSync(MATCH_HISTORY_KEY) || [];
      history.unshift(record);
      // 只保留最近50条记录
      if (history.length > 50) {
        history.length = 50;
      }
      uni.setStorageSync(MATCH_HISTORY_KEY, history);
    } catch (error) {
