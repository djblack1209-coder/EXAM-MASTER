...
export async function autoCollectMistake(question, userAnswer, aiAnalysis = '') {
  ...
  try {
    // 1. 获取现有错题本
    const mistakeBook = uni.getStorageSync(MISTAKE_BOOK_KEY) || [];
    ...
    if (existingIndex >= 0) {
      ...
      console.log('[MistakeAutoCollect] 更新已有错题，错误次数:', mistakeRecord.wrong_count);
    } else {
      // 新增
      mistakeBook.unshift(mistakeRecord);
      console.log('[MistakeAutoCollect] 新增错题');
    }
    
    // 6. 保存
    uni.setStorageSync(MISTAKE_BOOK_KEY, mistakeBook);
    
    // 7. 更新统计
    updateMistakeStats(classification);
    
    console.log('[MistakeAutoCollect] 错题收录成功:', {
      id: mistakeRecord.id,
...
function updateMistakeStats(classification) {
  try {
    const stats = uni.getStorageSync(MISTAKE_STATS_KEY) || {
      totalCount: 0,
      byCategory: {},
      byDifficulty: {},
      byErrorType: {},
    ...
    uni.setStorageSync(MISTAKE_STATS_KEY, stats);
...
export function getMistakeStats() {
  return uni.getStorageSync(MISTAKE_STATS_KEY) || {
    totalCount: 0,
    byCategory: {},
    byDifficulty: {},
    byErrorType: {},
...
export async function batchCollectMistakes(mistakes) {
  ...
  console.log('[MistakeAutoCollect] 批量收录完成:', {
