...
class OnboardingFlow {
  ...
  init() {
    ...
    try {
      // 恢复状态
      this.currentStep = storageService.get(STORAGE_KEYS.CURRENT_STEP) || ONBOARDING_STEPS.WELCOME;
      this.completedSteps = storageService.get(STORAGE_KEYS.COMPLETED_STEPS, []);
      this.events = storageService.get(STORAGE_KEYS.EVENTS, []);

      // 检查是否是新用户
      const firstVisitTime = storageService.get(STORAGE_KEYS.FIRST_VISIT_TIME);
      if (!firstVisitTime) {
        storageService.save(STORAGE_KEYS.FIRST_VISIT_TIME, Date.now());
        storageService.save(STORAGE_KEYS.IS_NEW_USER, true);
      }
   ...
   */
  isNewUser() {
    const isNew = storageService.get(STORAGE_KEYS.IS_NEW_USER);
  ...
  completeStep(step) {
    ...
    if (!this.completedSteps.includes(stepToComplete)) {
      ...
      storageService.save(STORAGE_KEYS.COMPLETED_STEPS, this.completedSteps);
    }
    ...
    if (nextIndex < ONBOARDING_CONFIG.stepOrder.length) {
      ...
      // 标记为非新用户
      storageService.save(STORAGE_KEYS.IS_NEW_USER, false);
    }

    storageService.save(STORAGE_KEYS.CURRENT_STEP, this.currentStep);

    // 记录事件
    this.trackEvent('step_completed', { step: stepToComplete, nextStep: this.currentStep });

    logger.log('[OnboardingFlow] 步骤完成:', stepToComplete, '-> 下一步:', this.currentStep);
  ...
  skipOnboarding() {
    ...
    logger.log('[OnboardingFlow] 引导已跳过');
  }
  ...
  reset() {
    ...
    storageService.save(STORAGE_KEYS.CURRENT_STEP, this.currentStep);
    storageService.save(STORAGE_KEYS.COMPLETED_STEPS, this.completedSteps);
    storageService.save(STORAGE_KEYS.EVENTS, this.events);
    storageService.save(STORAGE_KEYS.IS_NEW_USER, true);

    logger.log('[OnboardingFlow] 引导已重置');
  }
  ...
  trackEvent(eventName, data = {}) {
    ...
    logger.log('[OnboardingFlow] 事件记录:', eventName, data);
