...
class NetworkMonitor {
  ...
  _init() {
    ...
    console.log('[NetworkMonitor] 初始化完成')
  }
  ...
  configure(config) {
    ...
    console.log('[NetworkMonitor] 配置已更新:', this.config)
  }
  ...
  _checkNetworkStatus() {
    uni.getNetworkType({
      success: (res) => {
        ...
        console.log(`[NetworkMonitor] 当前网络: ${this.status.networkType} (${this.status.quality})`)
      },
      fail: (err) => {
  ...
  _setupNetworkListener() {
    uni.onNetworkStatusChange((res) => {
      ...
      console.log(`[NetworkMonitor] 网络变化: ${res.networkType} (连接: ${res.isConnected})`)
  ...
  _handleStatusChange(wasConnected, oldQuality) {
    ...
    if (wasConnected !== this.status.isConnected) {
      if (this.status.isConnected) {
        ...
        console.log(`[NetworkMonitor] 🌐 网络已恢复 (离线时长: ${Math.round(this.status.offlineDuration / 1000)}s)`)
    ...
    if (oldQuality && oldQuality !== this.status.quality) {
      ...
      if (this._isWeakNetwork(this.status.quality) && !this._isWeakNetwork(oldQuality)) {
        console.log('[NetworkMonitor] ⚠️ 检测到弱网环境')
   ...
   */
  _triggerSync() {
    console.log('[NetworkMonitor] 🔄 触发离线队列同步...')
  ...
  async _performPing() {
    ...
    const pingUrl = this.config.pingUrl || 'https://www.baidu.com/favicon.ico'
   ...
   */
  pauseSync() {
    console.log('[NetworkMonitor] ⏸️ 暂停数据同步')
    offlineQueue.pause()
  }
   ...
   */
  resumeSync() {
    console.log('[NetworkMonitor] ▶️ 恢复数据同步')
    offlineQueue.resume()
  }
  ...
  destroy() {
    ...
    this.isInitialized = false
    console.log('[NetworkMonitor] 已销毁')
  }
}
