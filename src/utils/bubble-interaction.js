...
class BubbleInteraction {
  ...
  init() {
    try {
      this.clickHistory = uni.getStorageSync(STORAGE_KEYS.CLICK_HISTORY) || [];
      this.interactionStats = uni.getStorageSync(STORAGE_KEYS.INTERACTION_STATS) || {};
    } catch (e) {
  ...
  recordClick(bubble) {
    ...
    console.log('[BubbleInteraction] 记录点击:', bubble.title);
  }
  ...
  saveData() {
    try {
      uni.setStorageSync(STORAGE_KEYS.CLICK_HISTORY, this.clickHistory);
      uni.setStorageSync(STORAGE_KEYS.INTERACTION_STATS, this.interactionStats);
    } catch (e) {
  ...
  clearHistory() {
    ...
    this.interactionStats = {};
    this.saveData();
    console.log('[BubbleInteraction] 历史已清除');
  }
}
