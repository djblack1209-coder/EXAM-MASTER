...
class AnalyticsService {
  ...
  init(options = {}) {
    ...
    // 恢复用户ID
    this.userId = uni.getStorageSync('user_id') || null;
    ...
    console.log('[Analytics] 初始化完成，会话ID:', this.sessionId);
  }
  ...
  track(eventName, data = {}, priority = EVENT_PRIORITY.NORMAL) {
    ...
    if (this.config.enableDebug) {
      console.log('[Analytics] 事件追踪:', eventName, data);
    }
  ...
  setUserProperties(properties) {
    const profile = uni.getStorageSync(STORAGE_KEYS.USER_PROFILE) || {};
    const updatedProfile = { ...profile, ...properties, updatedAt: Date.now() };
    uni.setStorageSync(STORAGE_KEYS.USER_PROFILE, updatedProfile);
   ...
   */
  getRetentionData() {
    return uni.getStorageSync(STORAGE_KEYS.RETENTION) || {
      firstVisit: null,
      lastVisit: null,
      visitDays: [],
      day1Retained: false,
   ...
   */
  getEventStats() {
    const events = uni.getStorageSync(STORAGE_KEYS.EVENTS) || [];
    const stats = {};
   ...
   */
  _getOrCreateSession() {
    const session = uni.getStorageSync(STORAGE_KEYS.SESSION);
    const now = Date.now();
    ...
    uni.setStorageSync(STORAGE_KEYS.SESSION, newSession);
  ...
  _saveEventQueue() {
    try {
      ...
      uni.setStorageSync(STORAGE_KEYS.EVENTS, eventsToSave);
    } catch (e) {
  ...
  _restoreEventQueue() {
    try {
      const savedEvents = uni.getStorageSync(STORAGE_KEYS.EVENTS) || [];
  ...
  async _uploadEvents(events) {
    ...
    try {
      ...
      uni.setStorageSync('analytics_uploaded', uploadedEvents);

      if (this.config.enableDebug) {
        console.log('[Analytics] 事件上报成功:', events.length, '条');
      }
    } catch (e) {
  ...
  _checkRetention() {
    ...
    uni.setStorageSync(STORAGE_KEYS.RETENTION, retention);
  }
}
