...
class AIAnalyticsService {
  ...
  getDailyStats() {
    try {
      const dailyStats = uni.getStorageSync(STORAGE_KEYS.AI_DAILY_STATS) || {}
  ...
  saveMetrics() {
    try {
      // 保存会话指标
      uni.setStorageSync(STORAGE_KEYS.AI_METRICS, {
        ...this.sessionMetrics,
        updatedAt: Date.now()
      })
      ...
      uni.setStorageSync(STORAGE_KEYS.AI_DAILY_STATS, dailyStats)
  ...
  restoreMetrics() {
    try {
      const saved = uni.getStorageSync(STORAGE_KEYS.AI_METRICS)
  ...
  resetSessionStats() {
    ...
    this.saveMetrics()
    console.log('[AIAnalytics] 会话统计已重置')
  }
  ...
  async uploadMetrics() {
    try {
      ...
      console.log('[AIAnalytics] 指标上报成功')
      return true
    } catch (e) {
