 ...
 */
function init() {
  // [PROD-SAFE] console.log('[TimerWorker] 初始化');
...
function start() {
  ...
  // [PROD-SAFE] console.log('[TimerWorker] 开始计时');
...
function stop() {
  ...
  // [PROD-SAFE] console.log('[TimerWorker] 停止计时');
...
function tick() {
  ...
  // [PROD-SAFE] console.log('[TimerWorker] Tick, 今日:', state.todayStudyTime);
...
function sync() {
  ...
  // [PROD-SAFE] console.log('[TimerWorker] 同步数据');
...
function handleReconnect(savedState) {
  ...
  if (savedState.sessionStartTime && savedState.lastTickTime) {
    ...
    if (offlineMinutes > 0 && offlineMinutes <= CONFIG.MAX_OFFLINE_COMPENSATION) {
      ...
      // [PROD-SAFE] console.log('[TimerWorker] 离线补偿:', offlineMinutes, '分钟');
    }
  }

  // [PROD-SAFE] console.log('[TimerWorker] 重连完成，今日:', state.todayStudyTime);
...
function setState(newState) {
  ...
  // [PROD-SAFE] console.log('[TimerWorker] 状态已更新');
}
...
// [PROD-SAFE] console.log('[TimerWorker] Worker 已启动');
 ...
 *   switch (type) {
 *     case 'tick':
 *       // [PROD-SAFE] console.log('今日学习时长:', state.todayStudyTime);
 *       break;
 *     case 'sync':
 *       // 同步到服务器
 *       syncToServer(state);
 *       break;
 *     case 'reconnect':
 *       // [PROD-SAFE] console.log('重连补偿:', compensation, '分钟');
 *       break;
 *   }
 * };
 *
