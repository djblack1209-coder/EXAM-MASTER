  ...
  if (isDev || issues.length > 0) {
    ...
    if (issues.length > 0) {
      ...
      issues.forEach(({ key, name, description }, index) => {
        logger.log(`\n  ${index + 1}. ${name}`);
        logger.log(`     用途: ${description}`);
        logger.log(`     设置: 在 .env.local 中添加 ${key}=your_value`);
      });

      logger.log('\n💡 快速配置步骤：');
      logger.log('   1. cp .env.example .env.local');
      logger.log('   2. 编辑 .env.local 填写配置值');
      logger.log('   3. 重启开发服务器');
    }
