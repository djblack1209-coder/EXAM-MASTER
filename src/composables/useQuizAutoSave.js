...
function doSaveProgress(progress) {
  try {
    ...
    uni.setStorageSync(QUIZ_PROGRESS_KEY, JSON.stringify(data));
    uni.setStorageSync(QUIZ_PROGRESS_TIMESTAMP_KEY, Date.now().toString());
    
    console.log('[QuizAutoSave] 进度已保存:', {
...
export function loadQuizProgress() {
  try {
    const dataStr = uni.getStorageSync(QUIZ_PROGRESS_KEY);
    const timestampStr = uni.getStorageSync(QUIZ_PROGRESS_TIMESTAMP_KEY);
    
    if (!dataStr || !timestampStr) {
      console.log('[QuizAutoSave] 无保存的进度');
      return null;
    }
    ...
    if (now - timestamp > PROGRESS_EXPIRE_TIME) {
      console.log('[QuizAutoSave] 进度已过期，清除');
      clearQuizProgress();
      return null;
    }
    ...
    console.log('[QuizAutoSave] 加载进度成功:', {
      currentIndex: data.currentIndex,
      seconds: data.seconds,
...
export function clearQuizProgress() {
  try {
    ...
    uni.removeStorageSync(QUIZ_PROGRESS_TIMESTAMP_KEY);
    console.log('[QuizAutoSave] 进度已清除');
    return true;
  } catch (error) {
