...
export function useStudyTimer(options = {}) {
  ...
  function init() {
    // 检查日期是否变化
    const savedDate = uni.getStorageSync(STORAGE_KEYS.STUDY_DATE);
    const today = new Date().toISOString().split('T')[0];
    
    if (savedDate !== today) {
      // 新的一天，重置今日时长
      todayStudyTime.value = 0;
      uni.setStorageSync(STORAGE_KEYS.STUDY_DATE, today);
      uni.setStorageSync(STORAGE_KEYS.TODAY_STUDY_TIME, 0);
      console.log('[StudyTimer] 新的一天，重置今日时长');
    } else {
      // 恢复今日时长
      todayStudyTime.value = uni.getStorageSync(STORAGE_KEYS.TODAY_STUDY_TIME) || 0;
    }
    
    // 恢复总时长
    totalStudyTime.value = uni.getStorageSync(STORAGE_KEYS.TOTAL_STUDY_TIME) || 0;
    
    // 检查是否有未完成的会话（断线重连）
    const savedSessionStart = uni.getStorageSync(STORAGE_KEYS.SESSION_START);
    if (savedSessionStart) {
      const elapsed = Math.floor((Date.now() - savedSessionStart) / 60000);
      if (elapsed > 0 && elapsed < 120) { // 最多补偿2小时
        console.log('[StudyTimer] 断线重连，补偿时长:', elapsed, '分钟');
    ...
    console.log('[StudyTimer] 初始化完成，今日:', todayStudyTime.value, '分钟');
  }
  ...
  function start() {
    ...
    // 保存会话开始时间（用于断线重连）
    uni.setStorageSync(STORAGE_KEYS.SESSION_START, sessionStartTime.value);
  ...
  function stop() {
    ...
    if (sessionStartTime.value) {
      ...
      console.log('[StudyTimer] 本次会话时长:', sessionDuration, '分钟');
    }
    
    // 清除会话开始时间
    uni.removeStorageSync(STORAGE_KEYS.SESSION_START);
    sessionStartTime.value = null;
    
    // 保存进度
    saveProgress();
    
    console.log('[StudyTimer] 停止计时');
  }
  ...
  function tick() {
    ...
    console.log('[StudyTimer] +1分钟，今日:', todayStudyTime.value);
  }
  ...
  function saveProgress() {
    try {
      uni.setStorageSync(STORAGE_KEYS.TODAY_STUDY_TIME, todayStudyTime.value);
      uni.setStorageSync(STORAGE_KEYS.TOTAL_STUDY_TIME, totalStudyTime.value);
      uni.setStorageSync(STORAGE_KEYS.STUDY_DATE, new Date().toISOString().split('T')[0]);
    } catch (e) {
    ...
    if (document.hidden) {
      ...
      if (savedSessionStart && isRunning.value) {
        ...
        if (compensation > 0 && compensation < 30) { // 最多补偿30分钟
          ...
          saveProgress();
          console.log('[StudyTimer] 页面恢复，补偿:', compensation, '分钟');
        }
      }
    }
    // #endif
  ...
  function addTime(minutes) {
    ...
    console.log('[StudyTimer] 手动添加:', minutes, '分钟');
  }
