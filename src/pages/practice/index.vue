    ...
    <view class="status-card">
      <div class="status-content">
        <div class="status-icon">
          <text class="emoji">{{ hasBank ? '📚' : '☁️' }}</text>
        </div>
        <div class="status-info">
          <h3 class="status-title">{{ hasBank ? '题库就绪' : '题库空空如也' }}</h3>
          <p class="status-desc">
            {{ hasBank ? `当前已收录 ${totalQuestions} 道真题` : '导入资料，AI 为您生成专属题库' }}
          </p>
        </div>
        <view class="status-actions" v-if="hasBank">
				...
				<div class="loading-content">
					...
					<div class="apple-ai-progress">
						<div class="apple-ai-progress-bar">
							<div class="apple-ai-progress-fill" :style="{ width: (generatedCount / totalQuestionsLimit) * 100 + '%' }"></div>
						</div>
					<p class="loading-step">
						{{ generatedCount === 0 ? '分析文档结构' : `已生成 ${getGeneratedQuestionCount()} 题 / 目标 ${totalQuestionsLimit * batchQuestionCount}` }}
					</p>
					</div>
