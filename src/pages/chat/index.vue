	...
	<view class="chat-container">
		<view class="custom-nav">
			...
			<view class="nav-center">
				...
				<text class="nav-subtitle">Apple Intelligence</text>
			</view>
		...
		<view class="function-buttons">
			<view class="function-btn" @click="handleMistakeAnalysis">
				<text class="btn-icon">📝</text>
				<text class="btn-text">错题分析</text>
			</view>
			<view class="function-btn" @click="handleKnowledgeQuery">
				<text class="btn-icon">❓</text>
				<text class="btn-text">知识答疑</text>
			</view>
		</view>
		...
		<view class="mistake-select-modal" v-if="showMistakeSelectModal">
			...
			<view class="modal-content-select">
				<view class="modal-header-select">
					<text class="modal-title-select">选择要分析的错题</text>
					<view class="modal-close-select" @click="closeMistakeSelect">
						<text class="modal-close-icon-select">×</text>
					</view>
				</view>
				<scroll-view scroll-y class="modal-scroll-select">
					<view 
						v-for="(mistake, index) in allMistakes" 
						:key="index" 
						class="mistake-item-select"
						:class="{ 'selected': selectedMistakeIndexes.includes(index) }"
						@click="toggleMistakeSelect(index)"
					>
						<view class="select-checkbox">
							<text class="checkbox-icon" v-if="selectedMistakeIndexes.includes(index)">✓</text>
						</view>
						<view class="mistake-content-select">
							<text class="mistake-category-select">{{ mistake.category || '未分类' }}</text>
							<text class="mistake-question-select">{{ mistake.question.substring(0, 80) }}{{ mistake.question.length > 80 ? '...' : '' }}</text>
						</view>
					</view>
				</scroll-view>
				<view class="modal-footer-select">
					<button class="modal-btn-select secondary" @click="closeMistakeSelect">取消</button>
					<button class="modal-btn-select primary" @click="confirmMistakeAnalysis" :disabled="selectedMistakeIndexes.length === 0">
			...
			<view class="message-list">
				...
				>
					...
					<view class="bubble-wrapper">
						...
						<text class="time-stamp" v-if="msg.showTime">{{ msg.time }}</text>
					</view>
				</view>
				
				<view class="message-row assistant" v-if="isTyping">
		...
		<view class="input-area-wrapper" :style="{ bottom: keyboardHeight + 'px' }">
			<view class="input-bar">
				...
				<view class="send-btn" :class="{ 'can-send': inputValue.trim() }" @click="handleSend">
...
.chat-container { 
	height: 100vh; 
	background-color: #F0F9ED; /* 基于 Wise Bright Green 的浅色背景 */
	display: flex; 
...
.nav-left { display: flex; align-items: center; color: #007AFF; font-size: 30rpx; }
.back-arrow { font-size: 36rpx; margin-right: 4rpx; }
.nav-center { display: flex; flex-direction: column; align-items: center; position: absolute; left: 50%; transform: translateX(-50%); }
.nav-title { font-size: 32rpx; font-weight: 700; color: #000; }
.nav-subtitle { font-size: 18rpx; color: #8E8E93; text-transform: uppercase; letter-spacing: 2rpx; }

/* 2. 消息列表 - 基于 Wise Bright Green 的浅色背景 */
.chat-body { flex: 1; background-color: #F0F9ED; /* 基于 Wise Bright Green 的浅色背景 */ }
.message-list { padding: 24rpx 20rpx; }
.message-row { display: flex; margin-bottom: 24rpx; align-items: flex-end; }
...
.assistant .bubble { 
	...
	color: #000000; /* 深色文字（纯黑，确保在白色背景上清晰可见） */
	border-radius: 20rpx 20rpx 20rpx 6rpx;
	border: 1px solid #E0E0E0; /* 浅灰色边框，确保在浅绿背景上可见 */
	box-shadow: 0 2rpx 8rpx rgba(0, 0, 0, 0.08); /* 增强阴影，确保气泡突出 */
}
...
.assistant .bubble::after {
	...
	height: 16rpx;
	background-color: #FFFFFF; /* 白色背景 */
	border-left: 1px solid #E0E0E0; /* 浅灰边框 */
...
.input-bar { display: flex; align-items: flex-end; gap: 12rpx; }
.input-inner { 
	flex: 1; 
	background: #F5F5F7; 
	border-radius: 24rpx; 
