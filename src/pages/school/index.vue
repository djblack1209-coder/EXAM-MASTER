<template>
	<view :class="['container', { 'dark-mode': isDark }]">
		...
		<scroll-view scroll-y class="main-scroll" :style="{ paddingTop: '24px' }">
			<!-- 步骤进度条 -->
			<view class="step-bar glass-card">
			...
			<view class="glass-card form-card" v-if="currentStep === 1">
				<view class="card-header">
					<text class="title">完善教育背景</text>
					<text class="subtitle">AI 将基于您的背景分析上岸概率</text>
				</view>

				<view class="input-group">
					<text class="label">本科/专科毕业院校</text>
					<input class="glass-input" v-model="formData.school" placeholder="请输入学校名称" placeholder-class="ph-style" />
				</view>

				<view class="input-group">
					<text class="label">报考专业</text>
					<picker mode="selector" :range="majorOptions" @change="bindMajorChange">
						<view class="glass-input picker-val">
							<text :class="{ 'placeholder-text': !formData.targetMajor }">{{ formData.targetMajor || '点击选择报考专业' }}</text>
				...
				<view class="input-group">
					...
					<view class="tab-group">
						<view :class="['tab-item', { active: formData.masterType === 'academic' }]" @tap="formData.masterType = 'academic'">学硕</view>
						<view :class="['tab-item', { active: formData.masterType === 'professional' }]" @tap="formData.masterType = 'professional'">专硕</view>
					</view>
				</view>

				<view class="input-group">
					<text class="label">目前学历</text>
					<view class="tab-group">
						<view :class="['tab-item', { active: formData.degree === 'bk' }]" @tap="formData.degree = 'bk'">本科全日制</view>
						<view :class="['tab-item', { active: formData.degree === 'zk' }]" @tap="formData.degree = 'zk'">专科/同等学力</view>
					</view>
				</view>

				<view class="input-group">
					<text class="label">英语证明</text>
					<picker mode="selector" :range="englishCertificates" @change="bindEnglishCertChange">
						<view class="glass-input picker-val">
							<text :class="{ 'placeholder-text': !formData.englishCert }">{{ formData.englishCert || '点击选择英语证明' }}</text>
							<text class="picker-arrow">▼</text>
						</view>
					</picker>
				</view>

				<button class="primary-btn pulse-btn" @tap="submitForm">
					<text>✨</text>
					开始 AI 择校匹配
				</button>
			</view>
			...
			<view class="result-container" v-if="currentStep === 2">
				<view class="result-header">
					<text class="rh-title">推荐 {{ filteredSchools.length }} 所院校</text>
					<view class="rh-filter" @click="showFilter = true">
						<text>{{ activeFilterCount > 0 ? `已选(${activeFilterCount})` : '筛选' }}</text>
						<text class="filter-arrow">▼</text>
					</view>
				</view>

				<view class="glass-card school-card" v-for="(school, index) in filteredSchools" :key="school.id" @click="navToDetail(school.id)">
					<view class="sc-head">
						<!-- <image :src="school.logo" class="sc-logo" mode="aspectFit" /> -->
						<view class="sc-info">
							<view class="sc-name-row">
								<text class="sc-name">{{ school.name }}</text>
								<text class="sc-loc">📍 {{ school.location }}</text>
							</view>
							<view class="sc-tags">
								<text v-for="(tag, tIdx) in school.tags" :key="tIdx" class="tag-item">{{ tag }}</text>
							</view>
						</view>
						<view class="match-rate">
							<text class="mr-val">{{ school.matchRate }}%</text>
							<text class="mr-lbl">匹配度</text>
						</view>
					</view>
					...
					<view class="sc-majors">
						<view v-for="(major, mIdx) in school.majors" :key="mIdx" class="major-item">
							...
							<view class="mj-scores">
								<view class="score-col"><text class="sc-year">2025(预)</text><text class="sc-num high">{{ major.scores && major.scores[0] ? major.scores[0].total : '-' }}</text></view>
								<view class="score-col"><text class="sc-year">2024</text><text class="sc-num">{{ major.scores && major.scores[1] ? major.scores[1].total : '-' }}</text></view>
								<view class="score-col"><text class="sc-year">2023</text><text class="sc-num">{{ major.scores && major.scores[2] ? major.scores[2].total : '-' }}</text></view>
								<view class="score-col"><text class="sc-year">英语</text><text class="sc-num sub">{{ major.scores && major.scores[0] ? (major.scores[0].eng || '-') : '-' }}</text></view>
							</view>
						</view>
					</view>
					
					<view class="card-footer">
						<view class="cf-btn outline" @click.stop="navToDetail(school.id, true)">招生简章</view>
						<view class="cf-btn" 
		...
		<view class="filter-mask" v-if="showFilter" @click="showFilter = false">
			<view class="filter-panel glass-card" @click.stop>
				<view class="fp-header">
					<text class="fp-title">院校筛选</text>
					<text class="fp-reset" @click="resetFilter">重置</text>
				</view>
				
				<view class="fp-section">
					<text class="fp-label">所在地区</text>
				...
				<view class="fp-section">
					<text class="fp-label">院校层次</text>
					<view class="filter-tags">
						<view class="ft-item" :class="{ active: filter.tag === '' }" @click="filter.tag = ''">不限</view>
						<view class="ft-item" :class="{ active: filter.tag === '985' }" @click="filter.tag = '985'">985院校</view>
						<view class="ft-item" :class="{ active: filter.tag === '211' }" @click="filter.tag = '211'">211院校</view>
						<view class="ft-item" :class="{ active: filter.tag === '自划线' }" @click="filter.tag = '自划线'">34所自划线</view>
					</view>
				</view>

				<button class="fp-confirm-btn" @click="showFilter = false">查看结果</button>
			</view>
		</view>
	</view>
</template>
