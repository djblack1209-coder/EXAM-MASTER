  ...
  <view :class="['container', { 'dark-mode': isDark }]">
    ...
    <view class="search-bar">
      <view class="search-input-wrapper">
        ...
        />
        <text v-if="searchKeyword" class="clear-icon" @tap="clearSearch">
          ✕
        </text>
      </view>
      ...
      try {
        ...
        if (res.code === 0) {
          ...
          this.friendList = (res.data || []).map((friend) => ({
            ...
            accuracy: friend.correct_questions && friend.total_questions
    ...
    handlePKChallenge(friend) {
      ...
      uni.showModal({
        ...
        success: (res) => {
          if (res.confirm) {
            // 跳转到 PK 对战页面
            safeNavigateTo(`/pages/practice-sub/pk-battle?mode=friend&opponentId=${friend.uid}&opponentName=${encodeURIComponent(friend.nickname)}&opponentAvatar=${encodeURIComponent(friend.avatar || this.defaultAvatar)}&opponentScore=${friend.score || 0}`, {
              success: () => {
