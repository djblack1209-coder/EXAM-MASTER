    ...
    handlePKChallenge() {
      ...
      uni.showModal({
        ...
        success: (res) => {
          ...
          if (res.confirm) {
            safeNavigateTo(`/pages/practice-sub/pk-battle?mode=friend&opponentId=${this.friendInfo.uid}&opponentName=${encodeURIComponent(this.friendInfo.nickname)}&opponentAvatar=${encodeURIComponent(this.friendInfo.avatar)}&opponentScore=${this.friendInfo.score}`);
          }
        },
        fail: () => {
          this.isActionLoading = false;
