<template>
	<view :class="['container', { 'dark-mode': isDark }]">
		...
		<view class="top-bar">
			...
			<!-- H5 测试标识 -->
			<view style="text-align: center; padding: 10px; background: #2ECC71; color: white; font-size: 16px; font-weight: bold; margin: 0 20px 16px; border-radius: 8px;">
				✅ Hello Exam Master - H5 预览正常运行！
			</view>
			
			<!-- 用户信息卡片 -->
			<view class="user-greeting-card">
				<view class="avatar-box" @tap="handleUserClick">
					<image :src="isLoggedIn ? (userInfo.avatarUrl || defaultAvatar) : defaultAvatar" 
						   class="user-avatar" 
						   mode="aspectFill"
							...
							<view class="stats-grid">
								...
								<view class="glass-card stats-card card-3d" :style="cardStyle">
									...
									<view class="circle-progress">
										<svg width="80" height="80" viewBox="0 0 100 100">
											<circle cx="50" cy="50" r="40" stroke="#e9ecef" stroke-width="10" fill="none"/>
											<circle cx="50" cy="50" r="40" stroke="#2ECC71" stroke-width="10" fill="none"
													stroke-dasharray="251.2" :stroke-dashoffset="251.2 * (1 - accuracy / 100)"
								...
								<view class="glass-card stats-card card-3d" :style="cardStyle">
									...
									<view class="mini-bar-chart">
										<view v-for="(day, index) in weekStudyTime" :key="index" 
											  class="mini-bar" 
				...
				<view class="glass-card feature-item card-hover-effect" @tap="navToUniverse">
							...
							<scroll-view scroll-x class="heatmap-scroll" :show-scrollbar="false">
								<view class="heatmap-grid">
									<view v-for="(col, cIdx) in heatmapData" :key="cIdx" class="hm-col">
										<view v-for="(dayData, rIdx) in col" :key="rIdx" 
											  class="hm-cell" 
											  :class="'level-' + dayData.level"
											  @tap="showHeatmapDetail(dayData)">
										</view>
									</view>
								</view>
							</scroll-view>
					...
					<view class="glass-card task-card card-hover-effect">
						<view class="todo-list">
							<view class="todo-item" v-for="(item, index) in tasks" :key="index" @click="toggleTask(index)" :class="{ 'item-done': item.done }">
								<view class="checkbox"><view class="check-inner" v-if="item.done"></view></view>
...
.top-bar {
	...
	padding-bottom: 8px;
	margin-bottom: 16px; /* 增加与用户信息卡片的间距 */
	/* 移除胶囊按钮相关样式，避免遮挡 */
	position: relative;
	z-index: 10;
}
...
.logo-row {
	...
	align-items: center; /* 图标和文字中轴线完全对齐 */
...
.logo-text-main {
	font-size: 16px; /* 字号下调至 16px */
	font-weight: 600; /* 使用 font-weight: 600 */
	color: var(--text-main, var(--text-title)); /* 使用 var(--text-main) */
	letter-spacing: 0.5px;
	line-height: 1; /* 确保行高为1，与图标高度对齐 */
...
.user-greeting-card {
	...
	align-items: center;
	padding: 0 20px;
	margin-bottom: 24px; /* 增加底部间距，与Logo区域保持距离 */
	margin-top: 8px; /* 增加顶部间距，与top-bar保持距离 */
	transition: var(--transition);
}
...
.scroll-content-padding { 
	...
	box-sizing: border-box;
	min-height: 100%; /* Ensure content can scroll properly */
}
...
.countdown-box:hover, .practice-entry:hover {
	transform: translateY(-2px);
	box-shadow: 0 4px 16px rgba(0, 0, 0, 0.12);
}
...
.bottom-spacer { 
	height: calc(140rpx + env(safe-area-inset-bottom)); /* Ensure enough space for bottom nav + safe area + last item visibility */
	min-height: 140rpx; /* Minimum height fallback */
}

/* 登录弹窗 */
.login-mask { 
	position: fixed; inset: 0; background: rgba(0,0,0,0.4); z-index: 2000; 
	display: flex; align-items: flex-end; backdrop-filter: blur(5px); 
}
.login-sheet { 
	width: 100%; background: var(--card-bg); border-radius: 24px 24px 0 0; 
	padding: 24px 24px 50px; text-align: center; animation: slideUp 0.3s; 
}
.sheet-title { 
	font-size: 18px; font-weight: 700; color: var(--text-title); display: block; margin-bottom: 30px; 
}
.avatar-chooser { 
	width: 72px; height: 72px; border-radius: 50%; padding: 0; margin: 0 auto 24px; 
	position: relative; background: var(--input-bg); border: 1px solid var(--card-border); 
	display: flex; align-items: center; justify-content: center; 
}
.avatar-chooser::after { border: none; }
.avatar-preview { width: 100%; height: 100%; border-radius: 50%; }
.nick-input { 
	width: 100%; background: var(--input-bg); height: 50px; border-radius: 12px; 
	text-align: center; margin-bottom: 24px; font-size: 15px; color: var(--text-title); 
	box-sizing: border-box; padding: 0 16px; border: 1px solid var(--card-border);
}
.primary-btn { 
	width: 100%; background: var(--accent-green); color: #FFF; border-radius: 25px; 
	font-weight: 700; font-size: 16px; height: 50px; display: flex; 
