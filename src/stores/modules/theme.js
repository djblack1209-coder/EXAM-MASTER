...
export const useThemeStore = defineStore('theme', () => {
    ...
    const setThemeType = (type) => {
        ...
        console.log('[ThemeStore] 主题已切换至:', type)
    }
    ...
    const setDarkMode = (dark) => {
        ...
        uni.setStorageSync('theme_type', themeType.value)

        // 触发全局深色模式更新事件
        uni.$emit('themeUpdate', dark ? 'dark' : 'light')
        uni.$emit('themeTypeUpdate', themeType.value)

        console.log('[ThemeStore] 深色模式:', dark ? '开启 (Bitget)' : '关闭 (Wise)')
    }
    ...
    const watchSystemTheme = () => {
        ...
        uni.onThemeChange((res) => {
            console.log('[ThemeStore] 系统主题变化:', res.theme)
