...
export const useLearningTrajectoryStore = defineStore('learningTrajectory', {
  ...
  actions: {
    ...
    init() {
      ...
      try {
        this.trajectory = uni.getStorageSync(STORAGE_KEYS.TRAJECTORY) || [];
        this.knowledgeMastery = uni.getStorageSync(STORAGE_KEYS.KNOWLEDGE_MASTERY) || {};
        this.sessions = uni.getStorageSync(STORAGE_KEYS.LEARNING_SESSIONS) || [];
        this.isInitialized = true;
        
        // 监听气泡点击事件
        uni.$on('bubble:clicked', this.handleBubbleClick.bind(this));
        
        console.log('[LearningTrajectory] 初始化完成');
      } catch (e) {
    ...
    recordEvent(type, data = {}) {
      ...
      console.log('[LearningTrajectory] 记录事件:', type, data);
    },
    ...
    startSession() {
      ...
      console.log('[LearningTrajectory] 开始会话:', session.id);
    ...
    endSession() {
      ...
      console.log('[LearningTrajectory] 结束会话:', session.id, '时长:', session.duration, '分钟');
    ...
    saveTrajectory() {
      try {
        uni.setStorageSync(STORAGE_KEYS.TRAJECTORY, this.trajectory);
      } catch (e) {
    ...
    saveMastery() {
      try {
        uni.setStorageSync(STORAGE_KEYS.KNOWLEDGE_MASTERY, this.knowledgeMastery);
      } catch (e) {
    ...
    saveSessions() {
      try {
        ...
        uni.setStorageSync(STORAGE_KEYS.LEARNING_SESSIONS, sessionsToSave);
      } catch (e) {
    ...
    clearAll() {
      ...
      uni.removeStorageSync(STORAGE_KEYS.TRAJECTORY);
      uni.removeStorageSync(STORAGE_KEYS.KNOWLEDGE_MASTERY);
      uni.removeStorageSync(STORAGE_KEYS.LEARNING_SESSIONS);
      
      console.log('[LearningTrajectory] 数据已清除');
    }
  }
});
