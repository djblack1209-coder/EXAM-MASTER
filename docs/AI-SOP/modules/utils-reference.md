...
> Last updated: 2026-03-25 (Round 39)

---

## 零、Chat 意图识别 (`chat/`) — Round 39 新增

| 文件                 | 导出                              | 说明                                                                            |
| -------------------- | --------------------------------- | ------------------------------------------------------------------------------- |
| `intent-detector.js` | `detectIntent(text)`              | 纯前端正则意图识别，支持 7 类意图（出题/复习/错题/计划/模考/查校/弱项），零延迟 |
|                      | `QUICK_ACTIONS`                   | 快捷动作列表（icon + label + text + action），用于 Chat 输入框上方 chip 按钮    |
|                      | `getIntentConfirmMessage(intent)` | 根据意图生成 AI 确认消息文本                                                    |
