...
- `npm run audit:question-bank:report`: writes `data/question-bank-release-audit.json` with public-course missing/pending coverage gaps, publishable source-manifest coverage, per-slot source candidate diagnostics, and published-card answer-evidence blockers. Public-course coverage counts only formal release banks; `usageScope=self_study_draft` remains practice-visible but is reported as pending, not published, and `pendingCoverageBlockerCount` keeps those slots release-blocking until they are verified. Source Manifest `official_paper` evidence with `sourceRole=answer` is answer evidence only and does not count as publishable paper coverage; `sourceRole` must be blank legacy combined evidence, `paper`, or `paper_answer` to satisfy a public-course source slot. Candidate samples preserve `sourceRole` and rank paper-like evidence ahead of answer-only evidence so operators do not pick an answer PDF as the next paper source to verify.
...
- `npm run audit:release:backlog`: merges the question-bank, flashcard-quality, external, WeChat DevTools, professional-index, and optional local source-audit reports into `data/release-blocker-backlog.json` plus `data/reports/release-blocker-backlog.md`; pass `-- --output <temp-json> --no-markdown` for read-only automation audits that must not refresh the repository Markdown report. Missing or structurally incomplete prerequisite reports are now emitted as `release_audit_inputs` P0 blockers instead of being treated as empty 0/0 coverage. Public-course missing/pending slots record whether they first need publishable official source evidence and why existing raw candidates are still blocked. The machine-readable report now includes `publicCourseSlotBacklog`, which aggregates coverage and source-evidence blockers by track-year before producing `nextBalancedPublicCourseSlots`, so operators can treat one public-course slot as one unit of work. Slot rows carry `sourceCandidateSamples` from Source Manifest, including `sourceRole` when available, and when `data/raw-inbox/public-course-2025/source-audit.json` exists or `--local-source-audit <path>` is passed, `localSourceAuditSummary`/`localSourceAuditSamples` for local PDF path, SHA-256, text-layer state, OCR need, blockers, and paper/answer companion completeness; slots with a loaded local audit but no matching file are reported as missing local paper/answer files instead of blank diagnostics. `sourceManifestRegistrationChecklist` turns those diagnostics into required Source Manifest fields, `verified_source_registry.py` input fields, local paper/answer file candidates, manifest candidate ids, and pre-registration blockers; `ready_for_human_verification` additionally includes human verification steps, post-registration commands, and `verifiedSourceRegistryDrafts` that operators can copy into `data/verified-source-registry.json` only after filling `sourceUrl`/`verifiedBy`/`evidenceNote` from human-verified authority checks. Those drafts include `expectedSha256`, `sourceRole`, `missingHumanFields`, and `humanFieldStatus` so `verified_source_registry.py` can reject a file whose bytes no longer match the local source audit, Source Manifest can preserve paper/answer role metadata, and operators can see that `DRAFT:` evidence notes still require replacement before merge. Blocked slots do not receive registry drafts. The checklist still requires human review and does not change release readiness. `summary.publicCourseLocalSourceSummary` aggregates those slot diagnostics into paired-readable, answer-blocked, missing-companion, OCR-needed, missing-local-file, and unavailable buckets for scheduler/operator dashboards. `summary.sourceManifestHumanRegistrationQueue` and top-level `sourceManifestHumanRegistrationQueue` expose only slots whose paper/answer files are readable and have registry drafts, including the next slot key, draft count, roles, local paths, expected SHA-256 values, local-file/SHA draft pairs, required human fields, unique missing human fields, instance-level missing human fields such as `paper.sourceUrl` and `answer.sourceUrl`, and follow-up commands; this queue is an operator handoff and still does not count as publishable source evidence. The Markdown report has a matching "Source Manifest 人工注册队列" table that pairs each local file with its expected SHA-256 and lists both unique missing fields and per-draft missing field instances, so operators can locate ready human-registration work without opening the full JSON or undercounting the fields to fill. The Markdown table lists multiple local samples and a registration checklist when paper/answer files are available, so operators do not need to open JSON just to find the companion answer SHA or the follow-up command chain. Local source-audit rows are diagnostic only: they identify the next local file to verify, but they do not count as publishable official source evidence and cannot change release readiness. The next action distinguishes paired readable paper/answer files that need human verification and Source Manifest registration from one-sided local files, partial/blocked answer files, PDFs that still require OCR or blocker repair, and slots that still need local paper/answer files. Candidate samples are ranked toward official papers, paper-like `sourceRole`, unblocked sources, verified/published status, matched answer evidence, and fewer blocker reasons, while local source-audit samples are ranked toward paper/paper+answer files with usable text layers and fewer blockers. The next-slot recommendation keeps pending public-course slots ahead of untouched missing slots, then round-robins across tracks inside each priority bucket so the queue does not collapse onto one subject when several tracks are equally actionable. The summary separates pending coverage blockers, raw `blockerItemCount`, and deduplicated `publicCourseBlockedSlotCount` because coverage and source-evidence rows can describe the same track-year slot. Non-course blockers use a separate operator table with workstream, target file, count, and next action.
