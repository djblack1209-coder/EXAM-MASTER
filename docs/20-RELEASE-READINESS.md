...
| 百度增量清洗队列 | `npm run baidu:cleaning:queue` 已生成 `data/cleaning-queue.json`：当前 `download_and_extract:completed=4`、`download_and_extract:failed=3`、`download_and_extract:pending=423`、`manual_review:pending=1178`；后续每日扫描只会把新增或 fingerprint 变化的文件重新置为 pending |
| 清洗执行器 | `scripts/baidu/run_cleaning_queue.py` 已能小批量消费 `download_and_extract` 任务，并安全加载 `.env` / `laf-backend/.env` 中的多 provider 号池；未知轨道/答案解析类任务已改为 `*-support-<sourceId>` 隔离输出，避免覆盖正式题面文件；本轮真实跑通英语 2000、英语一 2001 题面和对应答案解析 support 文件 |
| 清洗产物答案修复 | 新增 `npm run baidu:flashcards:repair:answers`，可按年份 + 题号从答案/解析 companion JSON 补缺失答案，并把 companion 标记为 `supportingEvidenceOnly`；已修复英语 2000 的 10 个缺答案和英语一 2001 的 11 个缺答案，全部保持 `answerEvidenceStatus=candidate_matched`，未伪装成可发布 `matched` |
| 清洗产物质量门禁 | `npm run baidu:flashcards:quality` / `npm run baidu:flashcards:quality:release` 已接入 `release:gate:report` / `release:gate`；当前 `canPromoteToPublic=false`、晋级候选 `files=2`、`cards=80`、`missingAnswers=0`、`sourceEvidenceBlockers=80`、`gradingBlockers=0`，另有 2 个 support 证据文件被隔离跳过 |
