...
- Promoted `math2:2019` through the deterministic Math II page-image bank path after release-priority advanced to `2019考研数学二真题.pdf`.
- Rechecked the local development environment: CodeGraph is callable with 632 indexed files, Node/npm/Python/rg/Poppler/tesseract are available, and the health collection script showed no current Codex MCP or skill blocker. The Claude-specific `.claude/settings.local.json` absence only limits that separate hook/MCP audit and does not block this Codex workflow.
- Added `scripts/cleaning/build_math2_2019_bank.py`, published `src/config/flashcard-banks/math2-2019.json`, generated 23 question crop assets plus a trimmed `answer-page-05.jpg` under `cdn-assets/question-bank/math2-2019`, registered the bank, and regenerated the compressed practice-bank table.
- Source audit: `src_021a329bfc22c3b26ed9b510-2019考研数学二真题.pdf` is a 5-page scanned PDF with no usable text layer; pages 1-4 are question pages and page 5 is a compact answer key. The published answer page excludes the promotional tail below q23; q9 retains a faint source watermark that does not contain answers.
...
- Promoted `math2:2015` through the deterministic Math II page-image bank path because the release backlog had a publishable official `paper_answer` candidate for this slot.
- Development environment note: CodeGraph reported 635 indexed files, 10,128 nodes, 24,254 edges, WAL mode enabled, and no pending index blocker. The health collection script classifies the repository as complex by file count, but no project-local Claude/Codex config issue blocks this workflow; system `python3` still lacks `python-dotenv`, so Baidu direct downloads should continue to use `.venv-baidu/bin/python`.
- Added `scripts/cleaning/build_math2_2015_bank.py`, published `src/config/flashcard-banks/math2-2015.json`, generated 23 question crop assets plus 14 trimmed answer-page assets under `cdn-assets/question-bank/math2-2015`, registered the bank, and regenerated the compressed practice-bank table.
- Source audit: `src_a37ec1fa555f79f247286bf4-2015考研数学二真题.pdf` is a 14-page scanned PDF with no usable text layer, SHA256 `b21fb079d30628c2a541edbd19119aa56def749bd9403caeeb74fbdc3b89c32b`. The source interleaves each prompt with answer analysis, so published question crops stop before answer labels while answer pages crop out recurring header/footer promotion.
- Validation result: `math2-2015.json` has 23 cards, `{single_choice:8, short_answer:15}`, section counts `{选择题:8, 填空题:6, 解答题:9}`, all cards carry `answerEvidenceStatus=matched`, q01=`D`, q02=`B`, q03=`A`, q08=`A`, q09=`48`, q10=`n(n-1)(ln2)^(n-2)`, q14=`21`, q15 contains `a=-1`, q16 contains `8/π`, q20=`30min`, and q23 answer evidence ends at `answer-page-14.jpg`.
- Crop QA recalibrated q08 after OCR found the previous crop included the q07解析尾部; the final OCR leakage scan over all 23 question images found no answer/analysis/promotion keyword hits. q22 answer follows the worked derivation on page 13 (`X=[[3,1,-2],[1,1,-1],[2,1,-1]]`) rather than the cramped answer label on page 12.
- Release gate result: 2005-2020 public-course `coverageGaps` dropped from 4 to 3, `publishedSlots` rose to 88, `sourceEvidenceGaps` stayed at 1, release backlog reports `blockers=5` and `publicCourseBlockedSlots=3`, and the remaining public-course slots are `math2:2016`, `english1:2018`, and `english1:2019`.

### 2026-06-03 Round 85

- Promoted `english1:2018` through the formal English I text-layer bank path, replacing the earlier local cleaned draft that had candidate evidence and a polluted q26 answer.
- Development environment note: CodeGraph is callable with 637 indexed files, 10,175 nodes, 24,343 edges, WAL mode enabled. The session's plugin metadata pointed at old `0732ec94` cache paths while the installed curated plugins were under `2abb1c44`, so compatibility symlinks were added in the local plugin cache; Superpowers skill files and CodeGraph are now resolvable.
- Added `scripts/cleaning/build_english1_2018_bank.py`, published `src/config/flashcard-banks/english1-2018.json`, generated 15 original-paper pages plus 15 answer-speed pages under `cdn-assets/question-bank/english1-2018`, registered the bank, and regenerated the compressed practice-bank table.
- Source audit: `src_c2b2d9106a7394b9ebf94bb2-2018年真题及答案速查.pdf` has a recoverable XRef warning but a usable 597-line text layer and official answer table. `src_65e38afdd4cb892710781e4f-2018年考研英语一真题.pdf` is retained as the original-paper visual audit source, and `src_7fc9c83e38b149770e57c209-2018考研英语一真题及解析.pdf` is retained as a document-version scan. The answer-speed page 16 promotional tail is excluded from released assets and card evidence.
- Validation result: `english1-2018.json` has 52 cards, `{single_choice:45, translation:5, essay:2}`, all cards carry `answerEvidenceStatus=matched`, q01=`C`, q21=`D`, q26=`D`, q30=`A`, Part B=`EGABD`, q46 contains `By the date of his birth Europe was witnessing the passing of the religious drama`, q51 is the graduation-ceremony email, and q52 is the `选课进行时` writing prompt. The bank blocks the old q26 passage-as-answer pollution plus known English II source bleed and promotional tail text.
- Release gate result: 2005-2020 public-course `coverageGaps` dropped from 3 to 2, `publishedSlots` rose to 89, `sourceEvidenceGaps` stayed at 1, release backlog reports `blockers=4` and `publicCourseBlockedSlots=2`, and the remaining public-course slots are `math2:2016` and `english1:2019`.

### 2026-06-03 Round 86

- Promoted `english1:2019` through the formal English I text-layer bank path, clearing the last English I 2005-2020 release coverage gap.
- Development environment note: CodeGraph remains callable with 639 indexed files, 10,234 nodes, and 24,467 edges; local Node/npm/Python/rg/Poppler tooling is available. The tracked worktree was clean before the round, and ignored raw PDFs, cleaning queues, audit reports, tmp renders, venvs, and caches remain outside the commit scope.
- Added `scripts/cleaning/build_english1_2019_bank.py`, published `src/config/flashcard-banks/english1-2019.json`, generated 15 original-paper pages plus 15 answer-speed pages under `cdn-assets/question-bank/english1-2019`, registered the bank, and regenerated the compressed practice-bank table.
- Source audit: `src_8de590b865dceaba46e1cf1f-2019年考研英语一真题.pdf` is retained as the original-paper visual audit source. The local ignored `data/raw-inbox/2019年真题及答案速查.pdf` has a usable text layer and matches manifest source `src_e5870f592977fc243c04658d`; its page 16 promotional tail is excluded from released assets and card evidence.
- Validation result: `english1-2019.json` has 52 cards, `{single_choice:45, translation:5, essay:2}`, all cards carry `answerEvidenceStatus=matched`, q01=`C`, q21=`A`, q26=`D`, q30=`B`, Part B=`EDGBA`, q46 contains `There is a great deal of this kind of nonsense in the medical journals`, q51 is the `Aiding Rural Primary Schools` email, and q52 is the picture-based writing prompt. The bank blocks the answer-speed promotional tail and known English II museum/chart source bleed.
- Release gate result: 2005-2020 public-course `coverageGaps` dropped from 2 to 1, `publishedSlots` rose to 90, `sourceEvidenceGaps` stayed at 1, release backlog reports `blockers=3` and `publicCourseBlockedSlots=1`; the only remaining public-course blocker is still `math2:2016`, whose current candidate is known mislabeled 2014 content.
