...
describe('App 启动流程', () => {
  ...
  it('Splash 页面 1.2s 后跳转首页', async () => {
    vi.useFakeTimers();
    const { openHomeTab } = await import('@/pages/splash/index.vue').then((m) => {
      // splash 使用 setup，模拟 onMounted
      return { openHomeTab: () => uni.switchTab({ url: '/pages/index/index' }) };
    });
...
describe('刷题中心', () => {
  ...
  it('questionService 初始化正常', async () => {
